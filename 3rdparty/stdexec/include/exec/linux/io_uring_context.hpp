...
#    ifdef STDEXEC_HAS_IO_URING_ASYNC_CANCELLATION
      ...
      static constexpr auto
        ...
        dur = std::clamp(dur, std::chrono::nanoseconds{0}, std::chrono::nanoseconds{999999999});
...
#    else
      ...
      static constexpr ::itimerspec
        ...
        __nsec -= __sec;
        __nsec =
          std::clamp(__nsec, std::chrono::nanoseconds{0}, std::chrono::nanoseconds{999999999});
        __timerspec.it_value.tv_sec += __sec.count();
        __timerspec.it_value.tv_nsec = __nsec.count();
        STDEXEC_ASSERT(
          0 <= __timerspec.it_value.tv_nsec && __timerspec.it_value.tv_nsec < 1000000000);
        return __timerspec;
      }
#    endif
