...
/// 3. pool_post_awaitable CPU-pool thread
    ...
    -> http::handler_fn
{
    return [&sctx](http::request_context& ctx) -> cn::task<void>
    {
        ...
        ctx.json(http::status::ok, std::format(R"({{"n":{},"fibonacci":{},"io_thread":"{}","pool_thread":"{}","back_thread":"{}"}})", n, result, io_tid, pool_tid, back_tid));
        co_return;
    };
}
