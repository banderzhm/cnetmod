...
inline auto field_to_json(const FieldValue& f) -> nlohmann::json
{
    ...
    if (f.is_float())
        return f.get_float();
    if (f.is_date())
        return f.get_date().to_string();
    if (f.is_datetime())
        return f.get_datetime().to_string();
    return f.to_string();
}
...
inline auto to_double(const FieldValue& f) -> double
{
    if (f.is_double())
        return f.get_double();
    if (f.is_float())
        return static_cast<double>(f.get_float());
    if (f.is_int64())
        return static_cast<double>(f.get_int64());
    if (f.is_uint64())
        return static_cast<double>(f.get_uint64());
    // Implementation note.
...
static auto db_should_retry(const mysql::result_set& rs) -> bool
{
    if (!rs.is_err())
        return false;
    if (rs.error_code == 2006)
        return true; // MySQL server has gone away
    if (rs.error_code != 0 && mysql::is_fatal_error(rs.error_code))
        return true;
    ...
    if (msg == "not connected")
        return true;
    if (msg.find("failed to send") != std::string::npos)
        return true;
    if (msg.find("connection lost") != std::string::npos)
        return true;
    if (msg.find("no response") != std::string::npos)
        return true;
    return false;
}
...
/// ORM: create mysql_session
auto db_orm(cn::io_context* io = nullptr)
    -> cn::task<std::optional<std::pair<mysql::pooled_connection, orm::mysql_session>>>
{
...
auto handle_get_years(http::request_context& ctx) -> cn::task<void>
{
    ...
    orm::mysql_mapper_session session(conn->get(), g_mapper_registry);

    // ProjectMapper.selectDistinctYears
    auto result = co_await session.query_tuple<std::int64_t>(
        "ProjectMapper.selectDistinctYears",
        std::flat_map<std::string, orm::param_value>{});
    ...
    -> cn::task<orm::result_set>
{
    ...
    std::flat_map<std::string, orm::param_value> params;
    params["start_date"] = orm::param_value::from_string(std::string(start_date));
    params["end_date"] = orm::param_value::from_string(std::string(end_date));
...
auto handle_create_project(http::request_context& ctx) -> cn::task<void>
{
    ...
    auto tx_result = co_await db.transaction([&]() -> cn::task<void>
        {
            // Create client(ORM)
            auto client_qb = orm::mysql_select<Client>()
                                 .where("`wechat_id` = {}", {orm::param_value::from_string(client_wechat)})
                                 .limit(1);
            ...
            std::flat_map<std::string, orm::param_value> params;
            params["client_id"] = orm::param_value::from_int(client_id);
            params["title"] = orm::param_value::from_string(title);
            params["total_amount"] = orm::param_value::from_double(amount);
            params["commission"] = orm::param_value::from_double(commission);
            params["status"] = orm::param_value::from_string(status);
            params["created_date"] = orm::param_value::from_string(created);

            // Implementation note.
            if (!delivery.empty())
            {
                params["delivery_date"] = orm::param_value::from_string(delivery);
            }
            if (!remarks.empty())
            {
                params["remarks"] = orm::param_value::from_string(remarks);
            }
    ...
    auto tx_result = co_await db.transaction([&]() -> cn::task<void>
        {
            ...
            std::flat_map<std::string, orm::param_value> params;
            params["id"] = orm::param_value::from_int(project_id);
            params["client_id"] = orm::param_value::from_int(client_id);
            params["title"] = orm::param_value::from_string(title);
            params["total_amount"] = orm::param_value::from_double(amount);
            params["commission"] = orm::param_value::from_double(commission);
            ...
            {
                params["delivery_date"] = orm::param_value::from_string(""); // Implementation note: NULL.
            }
            if (!remarks.empty())
            {
                params["remarks"] = orm::param_value::from_string(remarks);
            ...
            // Expenses(ORM )
            auto del_exp = co_await db.remove(orm::mysql_delete<Expense>()
                    .where("`project_id` = {}", {orm::param_value::from_int(project_id)}));
            if (del_exp.is_err())
            {
    ...
    auto tx_result = co_await db.transaction([&]() -> cn::task<void>
        {
            ...
            auto file_qb = orm::mysql_select<ProjectFile>()
                               .where("`project_id` = {}", {orm::param_value::from_int(project_id)})
                               .limit(1);
            ...
            auto del_file = co_await db.remove(orm::mysql_delete<ProjectFile>()
                    .where("`project_id` = {}", {orm::param_value::from_int(project_id)}));
            if (del_file.is_err())
            {
...
auto handle_upload_files(http::request_context& ctx) -> cn::task<void>
{
    ...
    orm::mysql_mapper_session session(conn_r->get(), g_mapper_registry);
    std::flat_map<std::string, orm::param_value> params;
    params["id"] = orm::param_value::from_int(project_id);
    ...
    orm::mysql_session db(conn_r->get());
    ...
    ctx.json(201, std::format(R"({{"message":"{}\u4e2a\u6587\u4ef6\u4e0a\u4f20\u6210\u529f"}})", saved_count));
}
...
auto handle_delete_file(http::request_context& ctx) -> cn::task<void>
{
    ...
    auto rs = co_await db.find_by_id<ProjectFile>(
        orm::param_value::from_string(std::string(id_str)));
    if (rs.empty())
    {
...
auto handle_batch_delete_files(http::request_context& ctx) -> cn::task<void>
{
    ...
    auto tx_result = co_await db.transaction([&]() -> cn::task<void>
        {
            ...
            auto file_rs = co_await db.find(
                orm::mysql_select<ProjectFile>().where(std::format("`id` IN ({})", id_list)));
            ...
            auto del = co_await db.remove(
                orm::mysql_delete<ProjectFile>().where(std::format("`id` IN ({})", id_list)));
            if (del.is_err())
            {
                throw std::runtime_error(del.error_msg);
            }
    ...
    {
        ctx.json(200, std::format(R"({{"message":"{}\u4e2a\u6587\u4ef6\u6279\u91cf\u5220\u9664\u6210\u529f"}})", affected));
    }
}
...
auto handle_login(http::request_context& ctx) -> cn::task<void>
{
    ...
    if (username == g_cfg.admin_user && pass_hash == g_cfg.admin_pass_hash)
    {
        ...
        ctx.json(200, std::format(R"({{"status":"success","access_token":"{}","token_type":"Bearer",)" R"("expires_in":{},"user":{{"username":"{}","role":"admin"}}}})", token, g_cfg.jwt_expiry_seconds, username));
    }
    else
    {
...
auto handle_check_auth(http::request_context& ctx) -> cn::task<void>
{
    ...
    ctx.json(200, std::format(R"({{"authenticated":true,"user":{{"username":"{}","role":"admin"}}}})", json_escape(*user)));
}
...
auto handle_statistics(http::request_context& ctx) -> cn::task<void>
{
    ...
    auto to_month = [](const auto& f) -> std::string
    {
        if (f.is_date())
        {
            auto s = f.get_date().to_string();
            return s.substr(0, 7);
        }
        if (f.is_datetime())
        {
            auto s = f.get_datetime().to_string();
            return s.substr(0, 7);
        }
    ...
    for (auto& row : proj_rs.rows)
    {
        ...
        if (!client_name.empty())
            client_contrib[client_name] += amt;
    ...
    {
        clientContrib.push_back({{"name", name},
    ...
    {
        projectStatus.push_back({{"name", name},
            {"value", val}});
    }
...
auto handle_overall_stats(http::request_context& ctx) -> cn::task<void>
{
    ...
    orm::mysql_mapper_session session_a(conn_a.get(), g_mapper_registry);
    orm::mysql_mapper_session session_b(conn_b.get(), g_mapper_registry);
    std::flat_map<std::string, orm::param_value> params;
    params["start_date"] = orm::param_value::from_string(start_date);
    params["end_date"] = orm::param_value::from_string(end_date);
...
auto handle_client_search(http::request_context& ctx) -> cn::task<void>
{
    ...
    orm::mysql_mapper_session session(conn->get(), g_mapper_registry);
    std::flat_map<std::string, orm::param_value> params;
    params["wechat_id"] = orm::param_value::from_string(std::string(wechat_id));
    ...
    else
    {
        ...
        ctx.json(200, std::format(R"({{"name":"{}","wechat_id":"{}"}})", json_escape(name), json_escape(wechat)));
    }
}
...
auto handle_client_fuzzy(http::request_context& ctx) -> cn::task<void>
{
    ...
    orm::mysql_mapper_session session(conn->get(), g_mapper_registry);
    std::flat_map<std::string, orm::param_value> params;
    params["keyword"] = orm::param_value::from_string(std::string(q));
    ...
    {
        result.push_back({{"wechatId", wechat_id},
            {"name", name}});
    }
    ctx.json(200, result.dump());
}
...
auto handle_get_prompts(http::request_context& ctx) -> cn::task<void>
{
    ...
    auto rs = co_await db.find(
        orm::mysql_select<AiPrompt>().order_by("`is_default` DESC, `created_at` DESC"));
    conn.return_without_reset();
    if (rs.is_err())
    {
...
auto handle_delete_prompt(http::request_context& ctx) -> cn::task<void>
{
    ...
    auto rs = co_await db.remove(
        orm::mysql_delete<AiPrompt>().where(
    ...
    auto tx_result = co_await db.transaction([&]() -> cn::task<void>
        {
            ...
            // Batch( XML Mapper)
            auto reset_rs = co_await session.execute("AiMapper.clearDefaultPrompts", std::flat_map<std::string, orm::param_value>{});
            if (reset_rs.is_err())
            {
...
auto handle_ai_history(http::request_context& ctx) -> cn::task<void>
{
    ...
    auto result_obj = co_await session.query_tuple<
        ...
        std::string, std::optional<std::string>>("AiMapper.selectAnalysisHistoryWithPrompt", std::flat_map<std::string, orm::param_value>{});
    ...
    for (std::size_t i = 0; i < result_obj.data.size(); ++i)
    {
        ...
        json += std::format(
            R"({{"id":{},"start_date":"{}","end_date":"{}",)" R"("project_summary":"{}","created_at":"{}","prompt_name":"{}"}})",
            id,
            start_date,
            end_date,
...
auto handle_ai_history_detail(http::request_context& ctx) -> cn::task<void>
{
    ...
    orm::mysql_mapper_session session(conn->get(), g_mapper_registry);
    std::flat_map<std::string, orm::param_value> params;
    params["id"] = orm::param_value::from_int(std::stoll(std::string(id_str)));
    ...
    auto result_obj = co_await session.query_tuple<
        ...
        std::string, std::optional<std::string>, std::optional<std::string>>("AiMapper.selectAnalysisDetailById", params);
...
auto handle_ai_cache(http::request_context& ctx) -> cn::task<void>
{
    ...
    auto& [conn, db] = *orm_r;
    auto rs = co_await db.find(
        orm::mysql_select<AiAnalysisHistory>()
            .where("`start_date` = {} AND `end_date` = {}",
                {orm::param_value::from_string(std::string(start_date)),
                    orm::param_value::from_string(std::string(end_date))}));
    conn.return_without_reset();
    if (!rs.empty())
    {
        auto& h = rs.data[0];
        ctx.json(200, std::format(R"({{"cached":true,"data":{{"id":{},"analysis_result":"{}","created_at":"{}"}}}})", h.id, json_escape(h.analysis_result.value_or("")), h.created_at.value_or("")));
    }
    else
    {
...
auto handle_ai_analyze(http::request_context& ctx) -> cn::task<void>
{
    ...
    if (!no_cache)
    {
        ...
        if (cache_orm)
        {
            ...
            auto cache_rs = co_await cache_db.find(
                orm::mysql_select<AiAnalysisHistory>()
                    .where("`start_date` = {} AND `end_date` = {}",
                        {orm::param_value::from_string(start_date),
                            orm::param_value::from_string(end_date)}));
            ...
            if (!cache_rs.empty() && cache_rs.data[0].analysis_result.has_value())
            {
                ...
                if (!result.empty())
                {
                    ctx.json(200, std::format(R"({{"cached":true,"result":"{}"}})", json_escape(result)));
                    co_return;
                }
            }
        }
    ...
    {
        std::flat_map<std::string, orm::param_value> params;
        params["id"] = orm::param_value::from_int(prompt_id);
    ...
    if (prompt_template.empty())
    {
        auto result_obj = co_await session.query_tuple<std::int64_t, std::string>(
            "AiMapper.selectDefaultPromptTemplate", std::flat_map<std::string, orm::param_value>{});
    ...
    // Database handling.
    std::flat_map<std::string, orm::param_value> save_params;
    save_params["start_date"] = orm::param_value::from_string(std::string(start_date));
    save_params["end_date"] = orm::param_value::from_string(std::string(end_date));
    save_params["prompt_id"] = orm::param_value::from_int(final_prompt_id);
    save_params["analysis_result"] = orm::param_value::from_string(full_content);
    save_params["project_summary"] = orm::param_value::from_string(data_summary);
    save_params["projects_json"] = orm::param_value::from_string(projects_json);
...
{
    ctx.json(200, std::format(R"({{"api_base":"{}","model":"{}","max_tokens":{},"temperature":{:.1f},"configured":{}}})", json_escape(g_cfg.llm_api_base), json_escape(g_cfg.llm_model), g_cfg.llm_max_tokens, g_cfg.llm_temperature, g_cfg.llm_api_key.empty() ? "false" : "true"));
    co_return;
}
...
auto handle_ai_config_update(http::request_context& ctx) -> cn::task<void>
{
    ...
    if (!api_base.empty())
        g_cfg.llm_api_base = api_base;
    if (!api_key.empty())
        g_cfg.llm_api_key = api_key;
    if (!model.empty())
        g_cfg.llm_model = model;

    auto mt = j.value("max_tokens", 0.0);
    if (mt > 0)
        g_cfg.llm_max_tokens = static_cast<int>(mt);
    auto temp = j.value("temperature", 0.0);
    if (temp > 0)
        g_cfg.llm_temperature = temp;
    ...
    openai::client& ai_client) -> cn::task<void>
{
    ...
    if (!g_cfg.llm_api_key.empty())
    {
        ...
        if (cr)
            logger::info("[AI] Connected to {}", g_cfg.llm_api_base);
        else
            logger::warn("[AI] Connect deferred: {}", cr.error());
    }
    ...
    srv.use(cn::jwt_auth({
        ...
        {
            return verify_jwt(token).has_value();
        },
        .skip_paths = {
            "/api/login",
            "/api/logout",
