#include <cnetmod/config.hpp>

import std;
import cnetmod.core;
import cnetmod.core.net_init;
import cnetmod.core.log;
import cnetmod.coro;
import cnetmod.io;
import cnetmod.executor;

#ifdef CNETMOD_HAS_SSL
import cnetmod.core.ssl;
import cnetmod.core.dtls;
#endif

namespace cn = cnetmod;

#ifdef CNETMOD_HAS_SSL

auto run_server(cn::io_context& ctx,
                cn::ssl_context& ssl_ctx,
                std::uint16_t port) -> cn::task<void>
{
    auto sock_r = cn::socket::create(cn::address_family::ipv4, cn::socket_type::datagram);
    if (!sock_r) {
        std::println("DTLS_SERVER_ERROR socket {}", sock_r.error().message());
        ctx.stop();
        co_return;
    }
    auto sock = std::move(*sock_r);
    (void)sock.apply_options(cn::socket_options{.reuse_address = true});

    auto addr = cn::ip_address::from_string("127.0.0.1");
    auto bound = sock.bind(cn::endpoint{*addr, port});
    if (!bound) {
        std::println("DTLS_SERVER_ERROR bind {}", bound.error().message());
        ctx.stop();
        co_return;
    }

    std::println("DTLS_SERVER_READY {}", port);

    std::array<std::byte, 65536> first{};
    cn::endpoint peer;
    auto n = co_await cn::async_recvfrom(
        ctx, sock, cn::mutable_buffer{first.data(), first.size()}, peer);
    if (!n) {
        std::println("DTLS_SERVER_ERROR recv {}", n.error().message());
        ctx.stop();
        co_return;
    }

    cn::dtls_datagram_session session{
        ssl_ctx,
        ctx,
        sock,
        peer,
        cn::dtls_role::server,
    };
    session.queue_datagram(cn::const_buffer{first.data(), *n});

    auto hs = co_await session.async_handshake();
    if (!hs) {
        std::println("DTLS_SERVER_ERROR handshake {}", hs.error().message());
        ctx.stop();
        co_return;
    }
    std::println("DTLS_SERVER_HANDSHAKE_OK");

    std::array<std::byte, 4096> plain{};
    auto rd = co_await session.async_read(cn::mutable_buffer{plain.data(), plain.size()});
    if (!rd) {
        std::println("DTLS_SERVER_ERROR read {}", rd.error().message());
        ctx.stop();
        co_return;
    }

    auto wr = co_await session.async_write(cn::const_buffer{plain.data(), *rd});
    if (!wr) {
        std::println("DTLS_SERVER_ERROR write {}", wr.error().message());
        ctx.stop();
        co_return;
    }

    std::println("DTLS_SERVER_ECHOED {}", *wr);
    ctx.stop();
}

#endif

auto main(int argc, char** argv) -> int
{
#ifndef CNETMOD_HAS_SSL
    std::println("DTLS_SERVER_ERROR SSL support is disabled");
    return 1;
#else
    cn::net_init net;
    const auto cert = argc > 1 ? std::string_view{argv[1]} : std::string_view{"examples/test_ssl/cert.pem"};
    const auto key = argc > 2 ? std::string_view{argv[2]} : std::string_view{"examples/test_ssl/key.pem"};
    const auto port = argc > 3
        ? static_cast<std::uint16_t>(std::stoul(argv[3]))
        : static_cast<std::uint16_t>(56840);

    auto ssl_ctx_r = cn::ssl_context::dtls_server();
    if (!ssl_ctx_r) {
        std::println("DTLS_SERVER_ERROR context {}", ssl_ctx_r.error().message());
        return 1;
    }
    auto& ssl_ctx = *ssl_ctx_r;
    if (auto r = ssl_ctx.load_cert_file(cert); !r) {
        std::println("DTLS_SERVER_ERROR cert {}", r.error().message());
        return 1;
    }
    if (auto r = ssl_ctx.load_key_file(key); !r) {
        std::println("DTLS_SERVER_ERROR key {}", r.error().message());
        return 1;
    }

    auto ctx = cn::make_io_context();
    cn::spawn(*ctx, run_server(*ctx, ssl_ctx, port));
    ctx->run();
    return 0;
#endif
}
