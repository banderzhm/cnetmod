...
/// Demonstrates client::transaction and mysql_session::transaction
/// MySQL run, : mall
    ...
    auto rs = co_await cli.transaction([&]() -> cn::task<void>
        {
            ...
            // Implementation note.
            co_await cli.execute(
                "INSERT INTO tx_orders (user_id, product, amount, status) " "VALUES (LAST_INSERT_ID(), 'Laptop', 800.0, 'pending')");
...
// =============================================================================
// Demo 2: mysql_session::transaction (ORM)
// =============================================================================

auto demo_session_transaction(orm::mysql_session& db) -> cn::task<void>
{
    logger::info("Demo 2: mysql_session::transaction (ORM)");
...
auto demo_transaction_with_isolation(orm::mysql_session& db) -> cn::task<void>
{
    std::println("\n=== Demo 3: 带隔离级别的事务 ===");
...
auto demo_transaction_rollback(orm::mysql_session& db) -> cn::task<void>
{
    std::println("\n=== Demo 4: 事务回滚演示 ===");
...
auto main() -> int
{
    ...
    logger::init("mysql-transaction-example", logger::level::info);

    try
    {
        logger::info("MySQL transaction example starting");
        std::println("=== cnetmod: MySQL Transaction Demo ===");

        cn::net_init net;
        auto ctx = cn::make_io_context();
        logger::info("MySQL transaction I/O context ready");
        cn::spawn(*ctx, run(*ctx));
        ctx->run();
    }
    catch (const std::exception& e)
    {
        logger::critical("MySQL transaction example failed: {}", e.what());
        logger::shutdown();
        return 1;
    }
    catch (...)
    {
        logger::critical(
            "MySQL transaction example failed with an unknown exception");
        logger::shutdown();
        return 1;
    }
