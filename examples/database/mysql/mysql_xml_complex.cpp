...
auto main() -> int
{
    ...
    {
        // 1. Connect to MySQL
        cnetmod::mysql::connect_options opts;
        opts.host = "127.0.0.1";
        opts.port = 3306;
        opts.username = "root";
        ...
        param_context ctx1 = param_context::from_map({{"keyword", param_value::from_string("test")},
        ...
        for (int i = 1; i <= 3; ++i)
        {
            users_batch.push_back(param_context::from_map({{"name", param_value::from_string(std::format("BatchUser{}", i))},
        ...
        param_context ctx3 = param_context::from_map({{"currentTime", param_value::from_string("NOW()")}});

        std::vector<param_context> updates;
        updates.push_back(param_context::from_map({{"id", param_value::from_int(1)},
            {"status", param_value::from_int(2)}}));
        updates.push_back(param_context::from_map({{"id", param_value::from_int(2)},
        ...
        auto result4 = co_await session.query<User>("UserMapper.advancedFilter",
            param_context::from_map({{"namePattern", param_value::from_string("%test%")},
        ...
        {
            param_context ctx5 = param_context::from_map({{"keyword", param_value::from_string("test")},
