...
auto demo_create_table(mysql::client& db) -> cn::task<void>
{
    auto rs = co_await db.query(
        "CREATE TABLE IF NOT EXISTS `cms_help` (" "  `id` bigint(20) NOT NULL AUTO_INCREMENT," "  `category_id` bigint(20) DEFAULT NULL," "  `icon` varchar(500) DEFAULT NULL," "  `title` varchar(100) DEFAULT NULL," "  `show_status` int(1) DEFAULT NULL," "  `create_time` datetime DEFAULT NULL," "  `read_count` int(1) DEFAULT NULL," "  `content` text," "  PRIMARY KEY (`id`) USING BTREE" ") ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='帮助表'");
...
auto demo_insert(mysql::client& db) -> cn::task<void>
{
    ...
    auto rs1 = co_await db.execute(mysql::with_params(
        "INSERT INTO cms_help (category_id, icon, title, show_status, create_time, read_count, content) " "VALUES ({}, {}, {}, {}, NOW(), {}, {})",
        {
            P::from_int(1),
    ...
    auto rs2 = co_await db.execute(mysql::with_params(
        "INSERT INTO cms_help (category_id, icon, title, show_status, create_time, read_count, content) VALUES " "({}, {}, {}, {}, NOW(), {}, {}), " "({}, {}, {}, {}, NOW(), {}, {}), " "({}, {}, {}, {}, NOW(), {}, {})",
        {
            P::from_int(1),
            P::from_string("/icons/login.png"),
            P::from_string("忘记密码怎么办"),
            P::from_int(1),
            P::from_int(0),
            P::from_string("点击登录页「忘记密码」，通过手机验证码重置密码。"),

            P::from_int(2),
            P::from_string("/icons/order.png"),
            P::from_string("如何查看订单"),
            P::from_int(1),
            P::from_int(0),
            P::from_string("登录后进入「我的订单」页面，可查看所有历史订单。"),

            P::from_int(2),
            P::from_string("/icons/refund.png"),
...
auto demo_metadata(mysql::client& db) -> cn::task<void>
{
    // Implementation note.
    co_await db.query(
        "INSERT INTO cms_help (category_id, title, show_status, create_time, read_count, content) " "VALUES (1, 'metadata test', 1, NOW(), 42, 'hello')");

    auto rs = co_await db.query("SELECT * FROM cms_help LIMIT 1");
    if (rs.is_err())
    {
        print_result(rs, "METADATA");
        co_return;
    }
...
auto main() -> int
{
    ...
    logger::init("mysql-crud-example", logger::level::info);

    try
    {
        logger::info("MySQL CRUD example starting");
        std::println("=== cnetmod: MySQL CRUD (cms_help) ===");

        cn::net_init net;
        auto ctx = cn::make_io_context();
        logger::info("MySQL CRUD I/O context ready");
        cn::spawn(*ctx, run(*ctx));
        ctx->run();
    }
    catch (const std::exception& e)
    {
        logger::critical("MySQL CRUD example failed: {}", e.what());
        logger::shutdown();
        return 1;
    }
    catch (...)
    {
        logger::critical("MySQL CRUD example failed with an unknown exception");
        logger::shutdown();
        return 1;
    }
