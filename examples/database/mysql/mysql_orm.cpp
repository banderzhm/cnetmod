...
/// Demonstrates orm::mysql_session create_table / insert / find / update / remove
/// MySQL run, : mall
...
auto demo_ddl(orm::mysql_session& db) -> cn::task<void>
{
    // Delete()
...
auto demo_insert(orm::mysql_session& db) -> cn::task<void>
{
    // Implementation note.
    Article a1;
    ...
    if (r1.is_err())
    {
        std::println("  INSERT error: {}", r1.error_msg);
        co_return;
    }
    ...
    if (r2.is_err())
    {
        std::println("  INSERT MANY error: {}", r2.error_msg);
        co_return;
    }
...
auto demo_select(orm::mysql_session& db) -> cn::task<void>
{
    // Implementation note.
    auto all = co_await db.find_all<Article>();
    if (all.is_err())
    {
        std::println("  find_all error: {}", all.error_msg);
        co_return;
    }
    ...
    if (!all.data.empty())
    {
        ...
        auto by_id = co_await db.find_by_id<Article>(orm::param_value::from_int(first_id));
        if (by_id.ok())
...
auto demo_update(orm::mysql_session& db) -> cn::task<void>
{
    // Implementation note.
    auto all = co_await db.find_all<Article>();
    if (all.is_err() || all.empty())
        co_return;
    ...
    if (r.is_err())
    {
        std::println("  UPDATE error: {}", r.error_msg);
        co_return;
    }
    std::println("\n── UPDATE ── affected={}", r.affected_rows);

    // Implementation note.
    auto updated = co_await db.find_by_id<Article>(orm::param_value::from_int(a.id));
    if (updated.ok())
...
auto demo_delete(orm::mysql_session& db) -> cn::task<void>
{
    auto all = co_await db.find_all<Article>();
    if (all.is_err() || all.data.size() < 2)
        co_return;
    ...
    if (r1.is_err())
    {
        std::println("  REMOVE error: {}", r1.error_msg);
        co_return;
    }
    ...
    // ID delete
    auto first_id = all.data.front().id;
    auto r2 = co_await db.remove_by_id<Article>(orm::param_value::from_int(first_id));
    if (r2.is_err())
    {
        std::println("  REMOVE_BY_ID error: {}", r2.error_msg);
        co_return;
    }
...
auto demo_delete_builder(orm::mysql_session& db) -> cn::task<void>
{
    auto db_result = orm::mysql_delete<Article>()
                         .where("`status` = {}", {orm::param_value::from_int(0)});

    auto r = co_await db.remove(db_result);
    if (r.is_err())
    {
        std::println("  DELETE builder error: {}", r.error_msg);
        co_return;
    }
...
auto demo_uuid(orm::mysql_session& db) -> cn::task<void>
{
    ...
    // UUID query
    auto r = co_await db.find_by_id<Tag>(orm::param_value::from_string(t1.id.to_string()));
...
auto demo_snowflake(orm::mysql_session& db) -> cn::task<void>
{
...
auto demo_sync_schema(mysql::client& cli) -> cn::task<void>
{
    ...
    co_await cli.execute(
        "CREATE TABLE `orm_products` (" "  `id` BIGINT NOT NULL AUTO_INCREMENT," "  `name` VARCHAR(255) NOT NULL," "  `price` DOUBLE NOT NULL," "  `old_col` VARCHAR(100) DEFAULT NULL," "  PRIMARY KEY (`id`)" ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    ...
    else
    {
        ...
        {
            const char* act = chg.action == orm::mysql_column_change::action_t::add ? "ADD"
                : chg.action == orm::mysql_column_change::action_t::drop            ? "DROP"
                                                                                    : "MODIFY";
...
auto main() -> int
{
    ...
    logger::init("mysql-orm-example", logger::level::info);

    try
    {
        logger::info("MySQL ORM example starting");
        std::println("=== cnetmod: MySQL ORM Demo ===");

        cn::net_init net;
        auto ctx = cn::make_io_context();
        logger::info("MySQL ORM I/O context ready");
        cn::spawn(*ctx, run(*ctx));
        ctx->run();
    }
    catch (const std::exception& e)
    {
        logger::critical("MySQL ORM example failed: {}", e.what());
        logger::shutdown();
        return 1;
    }
    catch (...)
    {
        logger::critical("MySQL ORM example failed with an unknown exception");
        logger::shutdown();
        return 1;
    }
