...
auto main() -> int
{
    ...
    auto work = [&]() -> task<void>
    {
        ...
        for (int i = 1; i <= 5; ++i)
        {
            param_context item = param_context::from_map({{"id", param_value::from_int(i)}});
        ...
        auto result6 = co_await session.query<User>("UserMapper.findByRole",
            param_context::from_map({{"role", param_value::from_string("admin")}}));
        ...
        auto result7 = co_await session.query<User>("UserMapper.searchUsers",
            param_context::from_map({{"keyword", param_value::from_string("%test%")},
                {"status", param_value::from_int(1)},
        ...
        auto delete_result = co_await session.execute("UserMapper.deleteByStatus",
            param_context::from_map({{"status", param_value::from_int(0)}}));
