...
// Lifecycle notifications are emitted only after a CONNECT has been accepted
// and exactly once when that accepted transport is cleaned up.  The generated
// connection id distinguishes a reconnect using the same MQTT client id from
// an older transport which is still unwinding.
export using connection_observer =
    std::function<void(std::string_view client_id, std::uint64_t connection_id)>;

// Runs after authentication and client-id validation but before CONNACK.  It
// lets an embedding application reserve a real connection permit; returning
// false rejects the CONNECT without creating an MQTT session.
export using connection_admission_handler =
    std::function<bool(std::string_view client_id, std::uint64_t connection_id)>;
...
namespace detail {
    ...
    struct conn_state
    {
        ...
        conn_state(socket s, io_context& ctx, std::size_t delivery_cap,
            ...
            : sock(std::move(s)), io(ctx), delivery_ch(delivery_cap), write_ch(write_cap), write_wake_ch(1), cross_delivery_pending_limit(delivery_cap) {}
...
public:
    ...
    void set_connect_observer(connection_observer observer);
    void set_disconnect_observer(connection_observer observer);
    void set_connection_admission_handler(connection_admission_handler handler);
    void set_connection_release_observer(connection_observer observer);
...
private:
    ...
    connection_observer connect_observer_;
    connection_observer disconnect_observer_;
    connection_admission_handler connection_admission_handler_;
    connection_observer connection_release_observer_;
    bool running_ = false;
