...
if [[ "${GITHUB_ACTIONS:-}" != true ]]; then
    echo 'This fixture is restricted to an isolated GitHub Actions runner.' >&2
    exit 2
fi
...
docker exec "${mongo[0]}" mongosh --quiet --eval \
    'quit(rs.initiate({_id:"cnetmod",members:[{_id:0,host:"127.0.0.1:27017"},{_id:1,host:"127.0.0.1:27018"},{_id:2,host:"127.0.0.1:27019"}]}).ok ? 0 : 1)'
