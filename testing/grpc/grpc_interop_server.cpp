...
int main(int argc, char **argv) {
  ...
  auto [ptr, ec] = std::from_chars(port_text.data(),
  ...
  health.set("cnetmod.testing.grpc.EchoService",
             grpc::health::serving_status::serving);
  grpc::health::register_service(grpc_router, health);
  grpc::reflection::install_service(
      grpc_router,
      {"cnetmod.testing.grpc.EchoService", "grpc.health.v1.Health"});
