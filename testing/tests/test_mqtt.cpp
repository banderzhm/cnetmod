...
TEST(mqtt_versioned_codecs_enforce_their_wire_versions) {
    cnetmod::mqtt::connect_options options{
        .client_id = "versioned-client",
        .version = cnetmod::mqtt::protocol_version::v5,
    };

    cnetmod::mqtt::v3::connect_packet v3_connect{options};
    auto v3_wire = cnetmod::mqtt::v3::encode(v3_connect);
    ASSERT_TRUE(!v3_wire.empty());
    // Fixed header + remaining length + MQTT name length/name = 8 bytes.
    ASSERT_EQ(static_cast<unsigned char>(v3_wire[8]), 4u);

    cnetmod::mqtt::v5::connect_packet v5_connect{options};
    auto v5_wire = cnetmod::mqtt::v5::encode(v5_connect);
    ASSERT_TRUE(!v5_wire.empty());
    ASSERT_EQ(static_cast<unsigned char>(v5_wire[8]), 5u);

    cnetmod::mqtt::v3::publish_packet v3_publish{
        .topic = "v3/topic", .payload = "payload"};
    auto v3_publish_wire = cnetmod::mqtt::v3::encode(v3_publish);
    ASSERT_TRUE(!v3_publish_wire.empty());

    cnetmod::mqtt::v5::publish_packet v5_publish{
        .topic = "v5/topic", .payload = "payload"};
    auto v5_publish_wire = cnetmod::mqtt::v5::encode(v5_publish);
    ASSERT_TRUE(!v5_publish_wire.empty());
}
