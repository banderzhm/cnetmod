...
TEST(database_datetime_round_trip_is_timezone_independent)
{
    ...
    const auto value = cnetmod::database::datetime_from_unix_seconds(input);
    ASSERT_TRUE(value.has_value());

    const auto output = cnetmod::database::unix_seconds_from_datetime(*value);
...
TEST(orm_parameter_context_preserves_native_parameter_types)
{
    ...
    ASSERT_EQ(datetime_value.datetime_val.to_string(), "2026-09-17 08:31:57");
}
