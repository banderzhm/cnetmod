...
class schedule_sender {
  template <typename Receiver> struct schedule_op {
    using operation_state_concept = stdexec::operation_state_t;
    io_context *ctx_;
    Receiver rcvr_;

    void start() noexcept {
      // Zero coroutine overhead: post to event loop via function pointer
      // callback
...
template <typename T, typename Receiver> class task_op_state {
public:
  using operation_state_concept = stdexec::operation_state_t;

  task_op_state(task<T> &&t, Receiver rcvr)
      : task_(std::move(t)), rcvr_(std::move(rcvr)) {}
...
private:
  ...
  // Bridge coroutine: co_awaits the task, stores result, then delivers via
  // final_suspend
...
template <typename T> struct task_completion_sigs {
  using type =
      stdexec::completion_signatures<stdexec::set_value_t(T),
                                     stdexec::set_error_t(std::exception_ptr)>;
};

template <> struct task_completion_sigs<void> {
  using type =
      stdexec::completion_signatures<stdexec::set_value_t(),
                                     stdexec::set_error_t(std::exception_ptr)>;
};
...
public:
  ...
  explicit task_sender(task<T> t) noexcept : task_(std::move(t)) {}
...
template <typename T> struct manual_receiver {
  using receiver_concept = stdexec::receiver_t;

  std::optional<T> *value_;
  std::exception_ptr *error_;

  void set_value(T val) noexcept { value_->emplace(std::move(val)); }
  void set_error(std::exception_ptr e) noexcept { *error_ = std::move(e); }
...
/// void specialization
template <> struct manual_receiver<void> {
  using receiver_concept = stdexec::receiver_t;

  bool *done_;
  std::exception_ptr *error_;

  void set_value() noexcept { *done_ = true; }
  void set_error(std::exception_ptr e) noexcept { *error_ = std::move(e); }
  void set_stopped() noexcept {}
...
auto sync_wait_sender(task_sender<T> &&sender) -> T {
  if constexpr (std::is_void_v<T>) {
    ...
    auto op =
        std::move(sender).connect(detail::manual_receiver<void>{&done, &error});
    op.start();
    if (error)
    ...
    std::exception_ptr error;
    auto op =
        std::move(sender).connect(detail::manual_receiver<T>{&result, &error});
    op.start();
    if (error)
      std::rethrow_exception(error);
    if (!result)
