module cnetmod.orm.dynamic_sql;

import cnetmod.orm.sql_query_data;
import cnetmod.core.log;

#ifdef CNETMOD_ORM_DEBUG
    #define ORM_DEBUG_LOG(...) logger::debug(__VA_ARGS__)
#else
...
dynamic_sql_processor::dynamic_sql_processor(const format_options& opts)
    : opts_(opts) {}

auto dynamic_sql_processor::process(const xml_node& stmt_node,
    ...
    -> std::string
{
    ...
    case param_value::kind_t::date_kind:
        return std::format("{:04d}-{:02d}-{:02d}", val.date_val.year,
            val.date_val.month, val.date_val.day);
    case param_value::kind_t::datetime_kind:
        return std::format("{:04d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d}",
            val.datetime_val.year, val.datetime_val.month,
            val.datetime_val.day, val.datetime_val.hour,
            val.datetime_val.minute, val.datetime_val.second);
    case param_value::kind_t::time_kind:
        return std::format("{:02d}:{:02d}:{:02d}", val.time_val.hours,
            val.time_val.minutes, val.time_val.seconds);
    default:
        return "";
    }
}
...
auto dynamic_sql_processor::is_timestamp_field(std::string_view field_name)
    -> bool
{
    // Common timestamp field patterns
    return field_name.ends_with("_at") || field_name.ends_with("_time") ||
        field_name.ends_with("Time") || field_name.ends_with("Timestamp") ||
        field_name == "timestamp" || field_name == "created" ||
        field_name == "updated" || field_name == "deleted";
}

// Helper: Convert Unix timestamp to calendar_datetime using C++20 chrono
auto dynamic_sql_processor::unix_timestamp_to_datetime(std::int64_t timestamp)
    -> param_value
{
...
auto dynamic_sql_processor::eval_test(std::string_view test_expr,
    const param_context& ctx) const -> bool
{
    if (test_expr.empty())
        return true;
...
void dynamic_sql_processor::process_children(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
...
void dynamic_sql_processor::process_node(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
    auto& tag = node.tag;
    if (tag == "if")
    {
        process_if(node, ctx, fragments);
    }
    else if (tag == "where")
    {
        process_where(node, ctx, fragments);
    }
    else if (tag == "set")
    {
        process_set(node, ctx, fragments);
    }
    else if (tag == "trim")
    {
        process_trim(node, ctx, fragments);
    }
    else if (tag == "foreach")
    {
        process_foreach(node, ctx, fragments);
    }
    else if (tag == "choose")
    {
        process_choose(node, ctx, fragments);
    }
    else if (tag == "include")
    {
        process_include(node, ctx, fragments);
    }
    else if (tag == "bind")
    {
        process_bind(node, ctx, fragments);
    }
    else
    {
...
void dynamic_sql_processor::process_text(std::string_view text,
    const param_context& ctx)
{
    ...
    bool last_was_space =
        !sql_buf_.empty() &&
        std::isspace(static_cast<unsigned char>(sql_buf_.back()));
    ...
    while (i < text.size())
    {
        ...
        if (is_parameterized)
        {
                ...
                is_timestamp_field(param_name))
            {
                ...
                ORM_DEBUG_LOG("[DEBUG] Auto-converted timestamp field '{}' to datetime",
                    param_name);
            }
        ...
        else
        {
            ...
            ORM_DEBUG_LOG("[DEBUG] Direct substitution: ${{{}}} = '{}'", param_name,
                value_str);
        }
...
void dynamic_sql_processor::append_normalized(std::string_view text,
    bool& last_was_space)
{
    for (char c : text)
    {
...
void dynamic_sql_processor::process_if(const xml_node& node, param_context& ctx,
    const fragment_map& fragments)
{
...
void dynamic_sql_processor::process_where(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
    ...
    while (!content.empty() &&
        std::isspace(static_cast<unsigned char>(content.front())))
        content.erase(0, 1);
...
void dynamic_sql_processor::process_set(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
    auto before_size = sql_buf_.size();
...
void dynamic_sql_processor::process_trim(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
    ...
    sql_buf_.resize(before_size);
    if (!prefix.empty())
        sql_buf_.append(prefix);
    sql_buf_.append(content);
    if (!suffix.empty())
        sql_buf_.append(suffix);
}

void dynamic_sql_processor::process_foreach(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
    ...
    for (auto& item_ctx : *collection)
    {
        ...
        // This allows #{id} to work when item="id" and the item context contains
        // {"id": value}
    ...
    if (!close.empty())
        sql_buf_.append(close);
}

void dynamic_sql_processor::process_choose(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
    for (auto& child : node.children)
...
void dynamic_sql_processor::process_include(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
    auto refid = node.attr("refid");
    ORM_DEBUG_LOG("[DEBUG process_include] refid='{}', fragments.size()={}",
        refid, fragments.size());

    auto it = fragments.find(std::string(refid));
    if (it != fragments.end())
    {
        ORM_DEBUG_LOG(
            "[DEBUG process_include] Found fragment, tag='{}', children.size()={}",
...
void dynamic_sql_processor::process_bind(const xml_node& node,
    param_context& ctx,
    const fragment_map& fragments)
{
    ...
    if (name.empty() || value_expr.empty())
        return;
...
auto dynamic_sql_processor::strip_leading(std::string s,
    ...
    -> std::string
{
    ...
    while (i < prefixes.size())
    {
        ...
        auto token =
            prefixes.substr(i, pipe == std::string_view::npos ? pipe : pipe - i);
...
auto dynamic_sql_processor::strip_trailing(std::string s,
    ...
    -> std::string
{
    ...
    while (i < suffixes.size())
    {
        ...
        auto token =
            suffixes.substr(i, pipe == std::string_view::npos ? pipe : pipe - i);
