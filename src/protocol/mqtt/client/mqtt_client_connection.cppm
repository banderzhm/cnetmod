...
public:
  ...
  auto publish(std::string_view topic, std::string_view payload,
      ...
      -> task<std::expected<void, std::string>>;
  auto subscribe(std::vector<subscribe_entry> entries,
                 const properties &props = {})
      -> task<std::expected<std::vector<std::uint8_t>, std::string>>;
  auto subscribe(std::string topic_filter, qos max_qos = qos::at_most_once,
                 const properties &props = {})
      -> task<std::expected<std::vector<std::uint8_t>, std::string>>;
  ...
  auto send_auth(std::uint8_t reason_code = 0, const properties &props = {})
      -> task<std::expected<void, std::string>>;
