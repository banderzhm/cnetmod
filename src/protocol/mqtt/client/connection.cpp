...
struct client::impl
{
    ...
    void close() noexcept;
    auto publish(std::string_view topic, std::string_view payload, qos q,
        bool retain, const properties& props)
    ...
    auto subscribe(std::string topic_filter, qos max_qos, const properties& props)
        -> task<std::expected<std::vector<std::uint8_t>, std::string>>;
    auto unsubscribe(std::vector<std::string> topic_filters,
        const properties& props)
        -> task<std::expected<void, std::string>>;
    auto disconnect(std::uint8_t reason_code, const properties& props)
        -> task<std::expected<void, std::string>>;
    ...
    auto send_auth(std::uint8_t reason_code, const properties& props)
        -> task<std::expected<void, std::string>>;
    ...
    void prepare_pending_ack(std::uint16_t pid,
        control_packet_type expected_type);
    void prepare_pending_suback(std::uint16_t pid);
    void complete_pending(std::uint16_t pid, control_packet_type type,
    ...
    std::set<std::uint16_t> qos2_received_;
    std::map<std::uint16_t, publish_message> qos2_pending_publish_;
...
client::impl::impl(io_context& ctx) noexcept
    : ctx_(ctx) {}

auto client::impl::connect(connect_options opts)
    -> task<std::expected<void, std::string>>
{
    ...
    auto connect_r = co_await async_connect_happy_eyeballs(
        ctx_, opts.host, opts.port,
        cnetmod::happy_eyeballs_options{
            .connect_timeout = opts.connect_timeout,
        });
    if (!connect_r)
    {
        logger::error("mqtt client connect: {}:{} failed: {}", opts.host, opts.port,
            connect_r.error().message());
        co_return std::unexpected(std::string("connect failed: ") +
            connect_r.error().message());
    }
    ...
    auto wr =
        co_await do_write(const_buffer{connect_pkt.data(), connect_pkt.size()});
    if (!wr)
    {
        sock_.close();
        co_return std::unexpected(std::string("send CONNECT failed: ") +
            wr.error().message());
    }

    auto frame_r = co_await read_frame();
    if (!frame_r)
        co_return std::unexpected(std::string("read CONNACK failed: ") +
            frame_r.error());
    ...
    if (!connack_r)
        co_return std::unexpected(std::string("decode CONNACK: ") +
            connack_r.error());
    ...
    if (version_ == protocol_version::v5)
    {
        ...
        {
            sock_.close();
            co_return std::unexpected(std::string("CONNACK v5 error: reason_code=") +
    ...
    else
    {
        ...
        {
            sock_.close();
            co_return std::unexpected(std::string("CONNACK refused: ") +
...
auto client::impl::is_connected() const noexcept -> bool
{
    return connected_;
}

auto client::impl::session_present() const noexcept -> bool
{
    return session_present_;
}

auto client::impl::connack_properties() const noexcept -> const properties&
{
    return connack_props_;
}
...
auto client::impl::publish(std::string_view topic, std::string_view payload,
    qos q, bool retain, const properties& props)
    -> task<std::expected<void, std::string>>
{
    if (!connected_)
    {
    ...
    logger::debug("mqtt client publish topic={} qos={} retain={}", topic,
        to_string(q), retain);
        ...
        for (int wait_i = 0; wait_i < 3000; ++wait_i)
        {
            ...
            if (!connected_)
                co_return std::unexpected(
                    std::string("disconnected while waiting for inflight quota"));
    ...
    {
        pid = alloc_packet_id();
        prepare_pending_ack(pid, q == qos::at_least_once ? control_packet_type::puback : control_packet_type::pubrec);
    }
    ...
    if (max_packet_size_ > 0 && pkt.size() > max_packet_size_)
        co_return std::unexpected(
            std::string("packet exceeds server maximum_packet_size"));
    ...
    if (q != qos::at_most_once)
    {
        ...
        im.msg.props = props;
        im.expected_ack = (q == qos::at_least_once) ? control_packet_type::puback
    ...
    if (q == qos::at_least_once)
    {
        ...
        if (!ack_r)
            co_return std::unexpected(ack_r.error());
        co_return std::expected<void, std::string>{};
    }

    auto rec_r = co_await wait_for_ack(pid);
    if (!rec_r)
        co_return std::unexpected(rec_r.error());
    ...
    auto wr2 =
        co_await do_write(const_buffer{pubrel_pkt.data(), pubrel_pkt.size()});
    if (!wr2)
    {
        pending_acks_.erase(pid);
    ...
    if (!comp_r)
        co_return std::unexpected(comp_r.error());

    co_return std::expected<void, std::string>{};
}

auto client::impl::subscribe(std::vector<subscribe_entry> entries,
    const properties& props)
    -> task<std::expected<std::vector<std::uint8_t>, std::string>>
{
    if (!connected_)
...
auto client::impl::subscribe(std::string topic_filter, qos max_qos,
    const properties& props)
    -> task<std::expected<std::vector<std::uint8_t>, std::string>>
{
...
auto client::impl::unsubscribe(std::vector<std::string> topic_filters,
    const properties& props)
    -> task<std::expected<void, std::string>>
{
    if (!connected_)
...
auto client::impl::disconnect(std::uint8_t reason_code, const properties& props)
    -> task<std::expected<void, std::string>>
{
    if (!connected_)
        co_return std::expected<void, std::string>{};
...
void client::impl::on_message(message_callback cb)
{
    msg_cb_ = std::move(cb);
}

void client::impl::on_disconnect(disconnect_callback cb)
{
    disconnect_cb_ = std::move(cb);
}

void client::impl::on_auth(auth_callback cb)
{
    auth_cb_ = std::move(cb);
}

void client::impl::set_reconnect(reconnect_options opts)
{
    reconnect_opts_ = std::move(opts);
}

auto client::impl::send_auth(std::uint8_t reason_code, const properties& props)
    -> task<std::expected<void, std::string>>
{
    if (!connected_)
        co_return std::unexpected(std::string("not connected"));
...
auto client::impl::version() const noexcept -> protocol_version
{
    return version_;
}

auto client::impl::do_write(const_buffer buf)
    -> task<std::expected<std::size_t, std::error_code>>
{
...
auto client::impl::do_read(mutable_buffer buf)
    -> task<std::expected<std::size_t, std::error_code>>
{
#ifdef CNETMOD_HAS_SSL
    if (ssl_)
...
void client::impl::prepare_pending_ack(std::uint16_t pid,
    control_packet_type expected_type)
{
...
void client::impl::complete_pending(std::uint16_t pid, control_packet_type type,
    std::string_view payload)
{
    ...
    -> task<std::expected<ack_result, std::string>>
{
    ...
    struct ack_awaitable
    {
        ...
        auto await_ready() noexcept -> bool
        {
            ...
            return !self.connected_ || it == self.pending_acks_.end() ||
                it->second.completed;
        }
        ...
        {
            auto it = self.pending_acks_.find(pid);
            if (!self.connected_ || it == self.pending_acks_.end() ||
                it->second.completed)
            {
                return h;
            }
    ...
    if (it != pending_acks_.end() && it->second.completed)
    {
        ...
        if (!ack)
            co_return std::unexpected(ack.error());
        co_return *ack;
    }
    ...
    -> task<std::expected<suback_result, std::string>>
{
    ...
    struct suback_awaitable
    {
        ...
        auto await_ready() noexcept -> bool
        {
            ...
            return !self.connected_ || it == self.pending_subacks_.end() ||
                it->second.completed;
        }
        ...
        {
            auto it = self.pending_subacks_.find(pid);
            if (!self.connected_ || it == self.pending_subacks_.end() ||
                it->second.completed)
            {
                return h;
            }
    ...
    pending_subacks_.erase(pid);
    co_return std::unexpected(
        std::string("disconnected while waiting for SUBACK"));
}

auto client::impl::send_ping() -> task<void>
{
    if (!connected_ || keep_alive_sec_ == 0)
        co_return;
...
auto client::impl::keep_alive_loop() -> task<void>
{
    ...
    ++active_loops_;
    auto interval =
        std::chrono::milliseconds(static_cast<int>(keep_alive_sec_ * 750));
    auto ping_wait =
        std::chrono::milliseconds(static_cast<int>(keep_alive_sec_ * 500));
    while (connected_)
    {
        ...
        if (ping_outstanding_)
        {
            ...
            connected_ = false;
            if (disconnect_cb_)
                disconnect_cb_("keep-alive timeout");
            close();
            break;
        }
        ...
        if (ping_outstanding_)
        {
            logger::warn("mqtt client PINGRESP timeout after {}ms",
                ping_wait.count());
            connected_ = false;
            if (disconnect_cb_)
                disconnect_cb_("PINGRESP timeout");
            close();
            break;
        }
    }
        ...
        for (auto it = inflight_out_.begin(); it != inflight_out_.end();)
        {
                ...
                it->expected_ack == control_packet_type::pubrec)
            {
                auto pkt = encode_publish(it->msg.topic, it->msg.payload,
                    it->msg.qos_value, it->msg.retain, true,
                    it->packet_id, version_, it->msg.props);
                auto wr = co_await do_write(const_buffer{pkt.data(), pkt.size()});
                if (!wr)
                {
                    connected_ = false;
            ...
            it->send_time = now;
            ++it->retry_count;
            logger::debug("mqtt client retry pid={} attempt={}", it->packet_id,
                it->retry_count);
            ++it;
        }
    }
...
auto client::impl::auto_reconnect_loop() -> task<void>
{
    ...
    while (reconnecting_)
    {
        if (reconnect_opts_.max_retries > 0 &&
            attempt >= reconnect_opts_.max_retries)
        {
            logger::warn("mqtt client auto-reconnect exhausted after {} attempts",
                attempt);
            reconnecting_ = false;
            if (disconnect_cb_)
                disconnect_cb_("reconnect exhausted");
            co_return;
        }

        ++attempt;
        logger::info("mqtt client auto-reconnect attempt={} delay={}ms", attempt,
            delay.count());
        ...
        if (r)
        {
            ...
            reconnecting_ = false;
            if (reconnect_opts_.restore_subscriptions &&
                !saved_subscriptions_.empty())
            {
...
auto client::impl::resend_inflight() -> task<void>
{
    for (auto& im : inflight_out_)
    {
        ...
        {
            auto pkt = encode_publish(im.msg.topic, im.msg.payload, im.msg.qos_value,
                im.msg.retain, true, im.packet_id, version_,
                im.msg.props);
            auto wr = co_await do_write(const_buffer{pkt.data(), pkt.size()});
            if (!wr)
                break;
...
auto client::impl::sleep_while_connected(std::chrono::milliseconds duration)
    -> task<void>
{
...
auto client::connect(connect_options opts)
    -> task<std::expected<void, std::string>>
{
    co_return co_await impl_->connect(std::move(opts));
}

auto client::is_connected() const noexcept -> bool
{
    return impl_->is_connected();
}

auto client::session_present() const noexcept -> bool
{
    return impl_->session_present();
}

auto client::connack_properties() const noexcept -> const properties&
{
    return impl_->connack_properties();
}
...
auto client::publish(std::string_view topic, std::string_view payload, qos q,
    bool retain, const properties& props)
    -> task<std::expected<void, std::string>>
{
    co_return co_await impl_->publish(topic, payload, q, retain, props);
}

auto client::subscribe(std::vector<subscribe_entry> entries,
    const properties& props)
    -> task<std::expected<std::vector<std::uint8_t>, std::string>>
{
    co_return co_await impl_->subscribe(std::move(entries), props);
}

auto client::subscribe(std::string topic_filter, qos max_qos,
    const properties& props)
    -> task<std::expected<std::vector<std::uint8_t>, std::string>>
{
    co_return co_await impl_->subscribe(std::move(topic_filter), max_qos, props);
}

auto client::unsubscribe(std::vector<std::string> topic_filters,
    const properties& props)
    -> task<std::expected<void, std::string>>
{
    co_return co_await impl_->unsubscribe(std::move(topic_filters), props);
}

auto client::disconnect(std::uint8_t reason_code, const properties& props)
    -> task<std::expected<void, std::string>>
{
    co_return co_await impl_->disconnect(reason_code, props);
}

void client::on_message(message_callback cb)
{
    impl_->on_message(std::move(cb));
}

void client::on_disconnect(disconnect_callback cb)
{
    impl_->on_disconnect(std::move(cb));
}

void client::on_auth(auth_callback cb)
{
    impl_->on_auth(std::move(cb));
}

void client::set_reconnect(reconnect_options opts)
{
    impl_->set_reconnect(std::move(opts));
}

auto client::send_auth(std::uint8_t reason_code, const properties& props)
    -> task<std::expected<void, std::string>>
{
    co_return co_await impl_->send_auth(reason_code, props);
}

auto client::version() const noexcept -> protocol_version
{
