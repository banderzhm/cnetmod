/// cnetmod.protocol.mqtt:topic_filter — MQTT Topic filter validation and
/// matching Supports +/# wildcards, follows MQTT v3.1.1 / v5.0 spec
...
export constexpr auto validate_topic_filter(std::string_view filter) noexcept
    -> bool {
...
export constexpr auto validate_topic_name(std::string_view name) noexcept
    -> bool {
  if (name.empty() || name.size() > 65535)
    return false;
  for (auto ch : name) {
    if (ch == '\0' || ch == '+' || ch == '#')
      return false;
...
inline auto split_topic(std::string_view topic)
...
export inline auto topic_matches(std::string_view filter,
