...
auto topic_alias_send::enabled() const noexcept -> bool
{
    return max_ > 0;
}

auto topic_alias_send::max() const noexcept -> std::uint16_t
{
    return max_;
}
...
auto topic_alias_recv::enabled() const noexcept -> bool
{
    return max_ > 0;
}

auto topic_alias_recv::max() const noexcept -> std::uint16_t
{
    return max_;
}
