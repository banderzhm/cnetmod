...
binary_data::binary_data(std::string_view data)
{
    assign(data);
}
...
binary_data::binary_data(const binary_data& other)
{
    copy_from(other);
}
...
auto binary_data::size() const noexcept -> std::size_t
{
    return size_;
}

auto binary_data::empty() const noexcept -> bool
{
    return size() == 0;
}
...
auto binary_data::str() const -> std::string
{
    return std::string(view());
}

binary_data::operator std::string_view() const noexcept
{
    return view();
}
