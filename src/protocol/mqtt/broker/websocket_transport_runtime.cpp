/// MQTT over WebSocket broker implementation.
module;
...
public:
    ...
    explicit ws_broker_impl(io_context& ctx)
        : ctx_(ctx), ws_server_(ctx) {}

    /// Multi-core mode
    explicit ws_broker_impl(server_context& sctx)
    ...
    void stop()
    {
        ...
        for (auto& [cid, ch] : online_channels_)
            ch->close();
...
private:
    ...
    auto handle_ws_connection(ws::ws_context& wctx) -> task<void>
    {
        ...
        auto& frame = *frame_r;
        if (frame.type != control_packet_type::connect)
            co_return;
        ...
        if (cd.client_id.empty())
        {
            ...
            {
                auto rc =
                    (cd.version == protocol_version::v5)
                    ? static_cast<std::uint8_t>(
        ...
        if (cd.version == protocol_version::v5)
        {
            for (auto& p : cd.props)
            {
                ...
                // Client topic_alias_maximum → upper limit of aliases server can send
                // to this client (not used yet)
                if (p.id == property_id::maximum_packet_size)
                    if (auto* v = std::get_if<std::uint32_t>(&p.value))
                        max_packet_size = static_cast<std::size_t>(*v);
            }
            // Server opts_.topic_alias_maximum → upper limit of aliases server can
            // receive (announced in CONNACK)
        ...
        if (cd.version == protocol_version::v5)
        {
            ...
            if (opts_.max_keep_alive > 0 && ss.keep_alive != original_keep_alive)
                connack_props.push_back(
                    {property_id::server_keep_alive, ss.keep_alive});
            // Receive Maximum
        ...
        co_await ws_write(
            wctx, encode_connack(session_present, rc, cd.version, connack_props));
        ...
        if (session_present)
        {
            for (auto& im : session->inflight_out)
            {
                ...
                {
                    co_await ws_write(wctx, encode_publish(im.msg.topic, im.msg.payload, im.msg.qos_value, false, true, im.packet_id, version, im.msg.props));
        ...
        while (connected && wctx.is_open())
        {
            ...
            if (!fr)
            {
                ...
                if (session)
                {
                    logger::info("mqtt ws unexpected disconnect client={}",
                        session->client_id);
                    ...
                    if (session->will_msg)
                    {
                        ...
                        if (will_delay > 0)
                        {
                            ...
                            spawn(ctx_,
                                will_delay_task(cid, std::move(will_copy), will_delay));
                        }
                        else
                        {
                            publish_message wp;
                            ...
                            {
                                auto topic = wp.topic;
                                retained_.store(topic,
                                    retained_message{topic, wp.payload.str(),
            ...
            case control_packet_type::connect:
                // MQTT spec: receiving a second CONNECT packet on the same connection
                // is a protocol error
                ...
                if (version == protocol_version::v5)
                {
                    co_await ws_write(
                        wctx,
                        encode_disconnect(
                            version, static_cast<std::uint8_t>(v5::disconnect_reason_code::protocol_error)));
                }
                connected = false;
                if (session)
                    co_await cleanup_session(session);
            ...
            case control_packet_type::publish:
            {
                ...
                if (!msg_r)
                {
                    ...
                    if (version == protocol_version::v5)
                        co_await ws_write(
                            wctx, encode_disconnect(version, static_cast<std::uint8_t>(v5::disconnect_reason_code::malformed_packet)));
                    connected = false;
                    if (session)
                        co_await cleanup_session(session);
                    delivery_ch.close();
                    ...
                    static_cast<std::uint8_t>(opts_.maximum_qos))
                {
                    ...
                    if (version == protocol_version::v5)
                        co_await ws_write(
                            wctx, encode_disconnect(version, static_cast<std::uint8_t>(v5::disconnect_reason_code::qos_not_supported)));
                    connected = false;
                    if (session)
                        co_await cleanup_session(session);
                    delivery_ch.close();
                ...
                if (session)
                {
                    ...
                    if (it != session->qos2_pending_publish.end())
                    {
                        ...
                        if (pending_msg.retain)
                        {
                            ...
                            retained_.store(topic,
                                retained_message{topic, pending_msg.payload.str(),
                                    pending_msg.qos_value,
                                    pending_msg.props});
                        }
            ...
            case control_packet_type::subscribe:
            {
                ...
                for (auto& entry : sub_r->entries)
                {
                    ...
                    if (security_.enabled())
                    {
                        ...
                        if (!security_.authorize_subscribe(session->username,
                                actual_filter))
                        {
                    ...
                    if (is_new || entry.rh == retain_handling::send)
                    {
                        ...
                        for (auto& rm : matches)
                        {
                            ...
                            co_await ws_write(wctx,
                                encode_publish(rm.topic, rm.payload, eff_qos,
                                    entry.retain_as_published, false,
                                    pid, version, rm.props));
                        }
                    }
                }

                co_await ws_write(wctx, encode_suback(sub_r->packet_id, return_codes, version, sub_r->props));
                break;
            }
            ...
            case control_packet_type::unsubscribe:
            {
                ...
                if (!unsub_r)
                {
                    logger::warn("mqtt ws decode UNSUBSCRIBE failed: {}",
                        unsub_r.error());
                    if (version == protocol_version::v5)
                        co_await ws_write(
                            wctx, encode_disconnect(version, static_cast<std::uint8_t>(v5::disconnect_reason_code::malformed_packet)));
                ...
                for (auto& tf : unsub_r->topic_filters)
                {
                    ...
                    if (shared && !shared->share_name.empty())
                    {
                        ...
                        shared_store_.remove_member(
                            shared->share_name, shared->topic_filter, session->client_id);
                    }
                    else
                    {
                ...
                co_await ws_write(
                    wctx, encode_unsuback(unsub_r->packet_id, version, reason_codes));
                break;
            }
            ...
            case control_packet_type::auth:
            {
                if (version == protocol_version::v5 && session && auth_handler_)
                {
                    ...
                    auto resp = auth_handler_(session->client_id, auth_r.reason_code,
                        auth_r.props);
                    if (resp)
...
ws_broker::ws_broker(io_context& ctx)
    : impl_(std::make_unique<ws_broker_impl>(ctx)) {}

ws_broker::ws_broker(server_context& ctx)
    : impl_(std::make_unique<ws_broker_impl>(ctx)) {}

ws_broker::~ws_broker() = default;

void ws_broker::set_options(ws_broker_options opts)
{
    impl_->set_options(std::move(opts));
}

void ws_broker::set_security(security_config cfg)
{
    impl_->set_security(std::move(cfg));
}

void ws_broker::set_auth_handler(broker_auth_handler handler)
{
    impl_->set_auth_handler(std::move(handler));
}

auto ws_broker::security() noexcept -> security_config&
{
    return impl_->security();
}

auto ws_broker::sessions() noexcept -> session_store&
{
    return impl_->sessions();
}

auto ws_broker::sessions() const noexcept -> const session_store&
{
    return impl_->sessions();
}

auto ws_broker::retained() noexcept -> retained_store&
{
    return impl_->retained();
}

auto ws_broker::retained() const noexcept -> const retained_store&
{
    return impl_->retained();
}

auto ws_broker::subscriptions() noexcept -> subscription_map&
{
    return impl_->subscriptions();
}

auto ws_broker::listen() -> std::expected<void, std::error_code>
{
    return impl_->listen();
}

auto ws_broker::listen(std::string_view host, std::uint16_t port,
    socket_options opts)
    -> std::expected<void, std::error_code>
{
    return impl_->listen(host, port, opts);
}

auto ws_broker::run() -> task<void>
{
    return impl_->run();
}
