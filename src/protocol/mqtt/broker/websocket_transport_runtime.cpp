...
public:
  ...
  explicit ws_broker_impl(io_context &ctx) : ctx_(ctx), ws_server_(ctx) {}
  ...
  /// Run
  auto run() -> task<void> { co_await ws_server_.run(); }
...
private:
  ...
  auto handle_ws_connection(ws::ws_context &wctx) -> task<void> {
    ...
    // Start delivery_loop
    spawn(ctx_, ws_delivery_loop(wctx, session, version, max_packet_size,
                                 delivery_ch));
    ...
    if (session_present) {
      for (auto &im : session->inflight_out) {
        if (im.expected_ack == control_packet_type::puback ||
          ...
          co_await ws_write(wctx, encode_publish(im.msg.topic, im.msg.payload,
                                                 im.msg.qos_value, false, true,
                                                 im.packet_id, version,
                                                 im.msg.props));
    ...
    while (connected && wctx.is_open()) {
      ...
      case control_packet_type::subscribe: {
        ...
        co_await ws_write(wctx, encode_suback(sub_r->packet_id, return_codes,
                                              version, sub_r->props));
        break;
      }
...
auto ws_broker::run() -> task<void> { return impl_->run(); }
void ws_broker::stop() { impl_->stop(); }
} // namespace cnetmod::mqtt
