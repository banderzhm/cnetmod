...
void finish_cross_worker_delivery(
...
auto detail::conn_state::do_write(std::string data, outbound_priority priority,
                                  bool batchable)
    -> task<std::expected<std::size_t, std::error_code>> {
  auto ticket_r = co_await submit_write(std::move(data), priority, batchable);
  if (!ticket_r)
    co_return std::unexpected(ticket_r.error());
...
auto detail::conn_state::submit_write(std::string data,
  ...
  auto size = data.size();
  auto completion =
      std::make_shared<channel<std::expected<std::size_t, std::error_code>>>(1);
...
auto detail::conn_state::raw_write(const std::string &data)
  ...
  if (ssl) {
    auto r =
        co_await ssl->async_write_all(const_buffer{data.data(), data.size()});
    if (!r)
...
#ifdef CNETMOD_HAS_SSL
  if (ssl)
    co_return co_await ssl->async_read(buf);
#endif
  co_return co_await async_read(io, sock, buf);
}

broker::broker(io_context &context) : ctx_(context) {}

broker::broker(server_context &context)
    : ctx_(context.accept_io()), sctx_(&context) {}

void broker::set_options(broker_options options) { opts_ = std::move(options); }

void broker::set_security(security_config config) {
  security_ = std::move(config);
}

void broker::set_auth_handler(broker_auth_handler handler) {
  auth_handler_ = std::move(handler);
}

void broker::set_publish_observer(publish_observer observer) {
  publish_observer_ = std::move(observer);
}

auto broker::security() noexcept -> security_config & { return security_; }

auto broker::sessions() noexcept -> session_store & { return sessions_; }

auto broker::sessions() const noexcept -> const session_store & {
  return sessions_;
}

auto broker::retained() noexcept -> retained_store & { return retained_; }

auto broker::retained() const noexcept -> const retained_store & {
  return retained_;
}

auto broker::subscriptions() noexcept -> subscription_map & { return sub_map_; }

auto broker::listen() -> std::expected<void, std::error_code> {
...
auto broker::metrics() const noexcept -> broker_metrics_snapshot {
  return broker_metrics_snapshot{
      ...
      .offline_enqueued_messages =
          offline_enqueued_messages_.load(std::memory_order_relaxed),
...
void broker::invalidate_route_cache() noexcept {
  std::atomic_store_explicit(&route_cache_,
                             std::shared_ptr<const route_cache_map>{
...
#ifdef CNETMOD_HAS_SSL
auto broker::listen_tls() -> std::expected<void, std::error_code> {
  if (opts_.tls_port == 0)
    return {};
...
auto broker::listen_tls(std::string_view host, std::uint16_t port,
  ...
  if (!ssl_ctx_r)
    return std::unexpected(ssl_ctx_r.error());
...
void broker::load_persistence_once() {
  if (!opts_.persistence_enabled || persistence_loaded_)
    return;
  ...
  if (sessions_r) {
    ...
    logger::warn("mqtt persistence load sessions failed: {}",
                 sessions_r.error());
  }
  ...
  if (retained_r) {
    ...
    logger::warn("mqtt persistence load retained failed: {}",
                 retained_r.error());
  }
}
...
auto broker::persistence_flush_loop() -> task<void> {
  ...
  while (running_) {
    ...
    if (!sr)
      logger::warn("mqtt persistence save sessions failed: {}", sr.error());
    auto rr = store.save_retained(retained_);
    if (!rr)
      logger::warn("mqtt persistence save retained failed: {}", rr.error());
  }
}
...
auto broker::keep_alive_watchdog(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  auto timeout =
      std::chrono::milliseconds(static_cast<int>(conn->keep_alive * 1500));
...
auto broker::read_frame(detail::conn_state &conn)
  ...
  while (true) {
    ...
    conn.parser.feed(
        std::string_view(reinterpret_cast<const char *>(tmp.data()), *r));
  }
}

auto broker::writer_loop(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  ...
  auto fail_pending_queues = [&](std::error_code ec) {
    for (auto &pkt : high_queue)
      fail_packet(pkt, ec);
    for (auto &pkt : normal_queue)
      fail_packet(pkt, ec);
    high_queue.clear();
    normal_queue.clear();
  ...
  while (true) {
    const auto max_messages =
        std::max<std::size_t>(1, opts_.write_batch_messages);
    ...
    if (!high_queue.empty()) {
      ...
      if (pkt.batchable) {
        const auto max_bytes =
            std::max<std::size_t>(data.size(), opts_.write_batch_bytes);
        ...
        while (messages < max_messages) {
          ...
          if (!peek.batchable || peek.priority != outbound_priority::normal)
            break;
          if (data.size() + peek.data.size() > max_bytes)
            break;
    ...
    if (!wr) {
      auto ec = wr.error();
      for (auto &p : pending)
        complete_one(p, std::unexpected(ec));
...
auto broker::handle_connection(socket client, io_context &io, bool use_tls)
    -> task<void> {
  ...
  while (conn->connected) {
    ...
    case control_packet_type::connect:
      logger::warn("mqtt duplicate CONNECT from client={}",
                   conn->session ? conn->session->client_id : "?");
      co_await send_disconnect_and_close(
          conn, static_cast<std::uint8_t>(
                    v5::disconnect_reason_code::protocol_error));
      co_return;
    case control_packet_type::publish:
      co_await handle_publish(*fr, conn);
      break;
    case control_packet_type::puback:
      co_await handle_puback(*fr, conn);
      break;
    case control_packet_type::pubrec:
      co_await handle_pubrec(*fr, conn);
      break;
    case control_packet_type::pubrel:
      co_await handle_pubrel(*fr, conn);
      break;
    case control_packet_type::pubcomp:
      co_await handle_pubcomp(*fr, conn);
      break;
    case control_packet_type::subscribe:
      co_await handle_subscribe(*fr, conn);
      break;
    case control_packet_type::unsubscribe:
      co_await handle_unsubscribe(*fr, conn);
      break;
    case control_packet_type::pingreq:
      co_await handle_pingreq(conn);
      break;
...
auto broker::delivery_loop(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  const auto max_batch_messages =
      std::max<std::size_t>(1, opts_.write_batch_messages);
  if (max_batch_messages == 1) {
    while (conn->connected) {
      ...
      auto pkt = encode_publish(msg.topic, msg.payload, msg.qos_value, false,
                                false, pid, conn->version, msg.props);

      if (conn->max_packet_size > 0 && pkt.size() > conn->max_packet_size) {
        if (pid != 0 && conn->session)
      ...
      if (msg.qos_value == qos::at_most_once) {
        auto queued = co_await conn->submit_write_queued(
            std::move(pkt), outbound_priority::normal, false);
        if (!queued) {
          write_failures_.fetch_add(1, std::memory_order_relaxed);
          conn->connected = false;
          break;
      ...
      auto wr = co_await conn->do_write(std::move(pkt),
                                        outbound_priority::normal, false);
      if (!wr) {
        if (msg.qos_value != qos::at_most_once && conn->session) {
          conn->session->release_packet_id(pid);
          std::erase_if(
              conn->session->inflight_out,
              [&](const inflight_message &im) { return im.packet_id == pid; });
        }
        write_failures_.fetch_add(1, std::memory_order_relaxed);
        conn->connected = false;
        break;
  ...
  const auto max_batch_bytes =
      std::max<std::size_t>(1, opts_.write_batch_bytes);
  ...
  auto flush_qos0_batch = [&]() -> task<bool> {
    ...
    auto queued = co_await conn->submit_write_queued(
        std::move(qos0_batch), outbound_priority::normal,
        opts_.write_batch_messages > 1, delivered_count);
    qos0_batch.clear();
  ...
  while (conn->connected) {
    ...
    if (max_batch_messages > 1) {
      (void)conn->delivery_ch.try_receive_many(batch, max_batch_messages - 1);
    }
    ...
    for (auto &msg : batch) {
      ...
      if (msg.qos_value == qos::at_most_once) {
        ...
        append_publish(qos0_batch, msg.topic, msg.payload, msg.qos_value, false,
                       false, 0, conn->version, msg.props);
        ++qos0_batch_messages;
...
auto broker::retry_loop(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  while (conn->connected) {
    ...
    for (auto it = inflight.begin(); it != inflight.end();) {
      auto elapsed =
          std::chrono::duration_cast<std::chrono::seconds>(now - it->send_time);
      if (elapsed < conn->session->retry_interval) {
        ++it;
        continue;
      }
      ...
      auto pkt = encode_publish(it->msg.topic, it->msg.payload,
                                it->msg.qos_value, false, true, it->packet_id,
                                conn->version, it->msg.props);
...
auto broker::handle_connect(const mqtt_frame &frame,
  ...
  if (!cd_r) {
    ...
    auto pkt =
        encode_connack(false,
                       static_cast<std::uint8_t>(
                           connect_return_code::unacceptable_protocol_version),
  ...
  if (security_.enabled()) {
    ...
    if (!auth_user) {
      ...
      auto rc = (cd.version == protocol_version::v5)
                    ? static_cast<std::uint8_t>(
                          v5::connect_reason_code::bad_user_name_or_password)
                    : static_cast<std::uint8_t>(
                          connect_return_code::bad_user_name_or_password);
  ...
  if (cd.client_id.empty()) {
    if (cd.clean_session) {
      ...
      auto rc = (cd.version == protocol_version::v5)
                    ? static_cast<std::uint8_t>(
                          v5::connect_reason_code::client_identifier_not_valid)
                    : static_cast<std::uint8_t>(
  ...
  bool session_present = false;
  {
    ...
    auto [ss_ref, present] =
        sessions_.create_or_resume(cd.client_id, cd.clean_session, cd.version);
    auto &ss = ss_ref;
  ...
  if (cd.version == protocol_version::v5) {
    if (cd_client_id_was_empty)
      connack_props.push_back(mqtt_property::string_prop(
          property_id::assigned_client_identifier, cd.client_id));
    if (opts_.topic_alias_maximum > 0)
      connack_props.push_back(
          {property_id::topic_alias_maximum, opts_.topic_alias_maximum});
    if (opts_.max_keep_alive > 0 && conn->keep_alive != cd.keep_alive)
      connack_props.push_back(
          {property_id::server_keep_alive, conn->keep_alive});
    if (opts_.receive_maximum < 65535)
      connack_props.push_back(
          {property_id::receive_maximum, opts_.receive_maximum});
    if (opts_.maximum_packet_size > 0)
      connack_props.push_back(
          {property_id::maximum_packet_size, opts_.maximum_packet_size});
    if (opts_.maximum_qos != qos::exactly_once)
      connack_props.push_back(mqtt_property::byte_prop(
          property_id::maximum_qos,
    ...
    if (!opts_.wildcard_sub_available)
      connack_props.push_back(mqtt_property::byte_prop(
          property_id::wildcard_subscription_available, 0));
    if (!opts_.sub_id_available)
      connack_props.push_back(mqtt_property::byte_prop(
          property_id::subscription_identifier_available, 0));
    if (!opts_.shared_sub_available)
      connack_props.push_back(mqtt_property::byte_prop(
          property_id::shared_subscription_available, 0));
  }
...
auto broker::handle_publish(const mqtt_frame &frame,
                            std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  ...
  if (!msg_r) {
    ...
    co_await send_disconnect_and_close(
        conn, static_cast<std::uint8_t>(
                  v5::disconnect_reason_code::malformed_packet));
    co_return;
  }
  auto &msg = *msg_r;
  ...
  if (static_cast<std::uint8_t>(msg.qos_value) >
    ...
    co_await send_disconnect_and_close(
        conn, static_cast<std::uint8_t>(
                  v5::disconnect_reason_code::qos_not_supported));
    co_return;
  }
  ...
  if (!validate_topic_name(msg.topic))
    co_return;
  ...
  if (msg.retain) {
    ...
    retained_.store(topic, retained_message{topic, msg.payload.str(),
                                            msg.qos_value, msg.props});
  }

  if (publish_observer_)
    publish_observer_(msg, conn->session
                               ? std::string_view{conn->session->client_id}
                               : std::string_view{});
  co_await route_publish(conn->io, msg,
                         conn->session ? conn->session->client_id : "");
}
auto broker::handle_puback(const mqtt_frame &frame,
                           std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
...
auto broker::handle_pubrec(const mqtt_frame &frame,
                           std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  auto ack_r = decode_ack(frame.payload, conn->version);
  if (!ack_r) {
...
auto broker::handle_pubrel(const mqtt_frame &frame,
                           std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  auto ack_r = decode_ack(frame.payload, conn->version);
  if (!ack_r) {
...
auto broker::handle_pubcomp(const mqtt_frame &frame,
                            std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  auto ack_r = decode_ack(frame.payload, conn->version);
  if (!ack_r) {
...
auto broker::handle_subscribe(const mqtt_frame &frame,
                              std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  ...
  for (auto &entry : sub.entries) {
    if (!validate_topic_filter(entry.topic_filter)) {
      return_codes.push_back(
          conn->version == protocol_version::v5
              ? static_cast<std::uint8_t>(
                    v5::suback_reason_code::topic_filter_invalid)
    ...
    if (security_.enabled()) {
      ...
      if (!security_.authorize_subscribe(conn->session->username,
                                         actual_filter)) {
        ...
        return_codes.push_back(
            conn->version == protocol_version::v5
                ? static_cast<std::uint8_t>(
                      v5::suback_reason_code::not_authorized)
                : static_cast<std::uint8_t>(suback_return_code::failure));
        continue;
      }
    }
...
auto broker::handle_unsubscribe(const mqtt_frame &frame,
                                std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  ...
  for (auto &tf : unsub.topic_filters) {
    bool existed = false;
    {
      ...
      if (shared && !shared->share_name.empty()) {
        ...
        shared_store_.remove_member(shared->share_name, shared->topic_filter,
                                    conn->session->client_id);
      } else {
    ...
    if (conn->version == protocol_version::v5)
      reason_codes.push_back(
          existed ? static_cast<std::uint8_t>(v5::unsuback_reason_code::success)
                  : static_cast<std::uint8_t>(
                        v5::unsuback_reason_code::no_subscription_existed));
  }
...
auto broker::handle_pingreq(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  (void)co_await conn->do_write(encode_pingresp(), outbound_priority::high);
}

auto broker::handle_disconnect(const mqtt_frame &frame,
                               std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  conn->connected = false;
  if (conn->session) {
...
auto broker::handle_auth(const mqtt_frame &frame,
                         std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  if (conn->version != protocol_version::v5)
    co_return;
  ...
  if (auth_handler_ && conn->session) {
    auto resp = auth_handler_(conn->session->client_id, auth_r.reason_code,
                              auth_r.props);
    if (resp) {
      (void)co_await conn->do_write(encode_auth(resp->first, resp->second),
                                    outbound_priority::high);
    }
  }
}
auto broker::handle_unexpected_disconnect(
    std::shared_ptr<detail::conn_state> conn) -> task<void> {
  conn->connected = false;
  if (conn->session) {
    logger::info("mqtt unexpected disconnect client={}",
                 conn->session->client_id);
    ...
    if (conn->session->will_msg) {
      ...
      if (will_delay > 0) {
        ...
        conn->session->will_msg.reset();
        spawn(conn->io,
              will_delay_task(conn->io, cid, std::move(will_copy), will_delay));
      } else {
...
auto broker::will_delay_task(io_context &io, std::string client_id,
                             will will_msg, std::uint32_t delay_sec)
    -> task<void> {
  ...
  publish_message wp;
  wp.topic = will_msg.topic;
  wp.payload = will_msg.message;
  wp.qos_value = will_msg.qos_value;
  wp.retain = will_msg.retain;
  wp.props = will_msg.props;
  if (wp.retain) {
    co_await state_mtx_.lock();
    async_lock_guard sg(state_mtx_, std::adopt_lock);
    auto topic = wp.topic;
    retained_.store(topic, retained_message{topic, wp.payload.str(),
                                            wp.qos_value, wp.props});
  }
  co_await route_publish(io, wp, client_id);
  logger::info("mqtt will delayed published client={}", client_id);
}

auto broker::publish_will(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  if (!conn->session || !conn->session->will_msg)
    co_return;
  auto &w = *conn->session->will_msg;
  publish_message wp;
  wp.topic = w.topic;
  wp.payload = w.message;
  wp.qos_value = w.qos_value;
  wp.retain = w.retain;
  wp.props = w.props;
  if (wp.retain) {
    co_await state_mtx_.lock();
...
auto broker::cleanup_connection(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  if (!conn->session)
    co_return;
  if (conn->cleanup_done)
    co_return;
  ...
  }
  {
    ...
    if (conn->session->clean_session &&
        conn->session->session_expiry_interval == 0) {
      sub_map_.erase_client(cid);
      shared_store_.remove_client(cid);
      sessions_.remove(cid);
      invalidate_route_cache();
...
auto broker::route_publish(io_context &origin_io, const publish_message &msg,
  ...
  if (!shared_present) {
    auto cache =
        std::atomic_load_explicit(&route_cache_, std::memory_order_acquire);
    if (cache) {
  ...
  if (targets.empty()) {
    ...
    for (auto &m : matches) {
      if (m.client_id == sender_cid && m.entry.no_local)
        continue;
  ...
  std::unordered_map<std::string, std::shared_ptr<detail::conn_state>>
      online_targets;
  ...
  for (auto &[cid, entry] : targets) {
    auto effective_qos =
        static_cast<qos>(std::min(static_cast<std::uint8_t>(msg.qos_value),
                                  static_cast<std::uint8_t>(entry.max_qos)));
    publish_message fwd;
    fwd.topic = msg.topic;
    fwd.payload = msg.payload;
    fwd.qos_value = effective_qos;
    fwd.retain = false;
    fwd.props = msg.props;

    if (entry.subscription_id > 0)
      fwd.props.push_back(
          {property_id::subscription_identifier, entry.subscription_id});

    auto ch_it = online_targets.find(cid);
    if (ch_it != online_targets.end()) {
      if (co_await deliver_to_connection(origin_io, ch_it->second,
                                         std::move(fwd))) {
        continue;
      }
    }
...
auto broker::deliver_retained(std::shared_ptr<detail::conn_state> conn,
  ...
  for (auto &rm : matches) {
    auto eq =
        static_cast<qos>(std::min(static_cast<std::uint8_t>(rm.qos_value),
                                  static_cast<std::uint8_t>(entry.max_qos)));
    std::uint16_t pid = 0;
    ...
    auto wr = co_await conn->do_write(
        encode_publish(rm.topic, rm.payload, eq, true, false, pid,
                       conn->version, rm.props),
        outbound_priority::normal,
        eq == qos::at_most_once && opts_.write_batch_messages > 1);
    if (!wr) {
      write_failures_.fetch_add(1, std::memory_order_relaxed);
      if (eq != qos::at_most_once && conn->session) {
        conn->session->release_packet_id(pid);
...
auto broker::deliver_inflight_resend(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  if (!conn->session)
    co_return;
  for (auto &im : conn->session->inflight_out) {
    if (im.expected_ack == control_packet_type::puback ||
      ...
      auto wr = co_await conn->do_write(
          encode_publish(im.msg.topic, im.msg.payload, im.msg.qos_value, false,
                         true, im.packet_id, conn->version, im.msg.props),
          outbound_priority::normal,
...
auto broker::deliver_offline_queue(std::shared_ptr<detail::conn_state> conn)
    -> task<void> {
  if (!conn->session)
    co_return;
  ...
  for (auto &msg : queue) {
    ...
    if (session_state::check_message_expiry(msg))
      continue;
    ...
    auto wr = co_await conn->do_write(
        encode_publish(msg.topic, msg.payload, msg.qos_value, false, false, pid,
                       conn->version, msg.props),
        outbound_priority::normal,
        msg.qos_value == qos::at_most_once && opts_.write_batch_messages > 1);
    if (!wr) {
      write_failures_.fetch_add(1, std::memory_order_relaxed);
      if (msg.qos_value != qos::at_most_once) {
        conn->session->release_packet_id(pid);
...
auto broker::send_disconnect_and_close(std::shared_ptr<detail::conn_state> conn,
