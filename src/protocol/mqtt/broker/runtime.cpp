...
broker::broker(io_context& context)
    : ctx_(context) {}

broker::broker(server_context& context)
    : ctx_(context.accept_io()), sctx_(&context) {}

void broker::set_options(broker_options options)
{
...
auto broker::sessions() noexcept -> session_store&
{
    return sessions_;
}
...
auto broker::retained() noexcept -> retained_store&
{
    return retained_;
}
...
auto broker::subscriptions() noexcept -> subscription_map&
{
    return sub_map_;
}
    ...
    while (conn->connected)
    {
        ...
        case control_packet_type::connect:
            ...
            co_await send_disconnect_and_close(
                conn, static_cast<std::uint8_t>(v5::disconnect_reason_code::protocol_error));
            co_return;
    ...
    -> task<void>
{
    ...
    if (!msg_r)
    {
        ...
        co_await send_disconnect_and_close(
            conn, static_cast<std::uint8_t>(v5::disconnect_reason_code::malformed_packet));
        co_return;
    }
    auto& msg = *msg_r;
        ...
        static_cast<std::uint8_t>(opts_.maximum_qos))
    {
        ...
        co_await send_disconnect_and_close(
            conn, static_cast<std::uint8_t>(v5::disconnect_reason_code::qos_not_supported));
        co_return;
    }
    ...
    if (msg.retain)
    {
        ...
        retained_.store(topic, retained_message{topic, msg.payload.str(), msg.qos_value, msg.props});
    }
    ...
    -> task<void>
{
    ...
    if (wp.retain)
    {
        ...
        async_lock_guard sg(state_mtx_, std::adopt_lock);
        auto topic = wp.topic;
        retained_.store(topic, retained_message{topic, wp.payload.str(), wp.qos_value, wp.props});
    }
