...
namespace {
    ...
    {
    public:
        explicit schema_parser(std::string source)
    ...
    private:
        ...
        {
            skip();
            if (eof() || !(std::isalpha(static_cast<unsigned char>(source_[pos_])) || source_[pos_] == '_'))
                return std::nullopt;
            const auto start = pos_++;
            while (!eof() && (std::isalnum(static_cast<unsigned char>(source_[pos_])) || source_[pos_] == '_' || source_[pos_] == '.'))
                ++pos_;
...
{
    put_varint(out, static_cast<std::uint64_t>(number) << 3 | static_cast<std::uint8_t>(type));
}
    ...
    -> const field*
{
    auto it = std::ranges::find_if(
        fields, [number](const field& value)
        {
...
{
    auto it = std::ranges::find_if(
        file.messages, [name](const auto& value)
        {
...
{
    auto it = std::ranges::find_if(
        file.services, [name](const auto& value)
        {
            return value.name == name;
        });
...
{
    auto it = std::ranges::find_if(
        service.rpcs, [name](const auto& value)
        {
            return value.name == name;
        });
