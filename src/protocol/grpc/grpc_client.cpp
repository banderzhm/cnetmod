...
void client::close() noexcept { http_.close(); }
...
auto response_messages(const http::response &resp, const client_options &opts)
  ...
  return frames_to_messages(
      *frames, codec_options{
                   .compression = encoding,
                   .accept_compressed = opts.accept_gzip &&
                                        encoding == compression_algorithm::gzip,
                   .max_message_bytes = opts.max_receive_message_bytes,
               });
}

auto response_unary_identity_payload(const http::response &resp,
  ...
  if (encoding != compression_algorithm::identity) {
    ...
    if (messages->size() > 1) {
      return std::unexpected(
          make_internal("unary grpc response returned multiple messages"));
    }
    if (messages->empty())
      return byte_buffer{};
  ...
  if (std::to_integer<unsigned char>(bytes[0]) != 0) {
    return std::unexpected(
        make_internal("compressed unary grpc frame missing grpc-encoding"));
  }

  auto n = (static_cast<std::uint32_t>(std::to_integer<unsigned char>(bytes[1]))
            << 24) |
...
auto validate_outbound(const streaming_request &req, const client_options &opts)
  ...
  if (metadata_wire_size(req.headers) > opts.max_metadata_bytes) {
    return std::unexpected(make_status(status_code::resource_exhausted,
                                       "grpc metadata exceeds send limit"));
  }
  for (const auto &msg : req.messages) {
    if (msg.size() > opts.max_send_message_bytes) {
      return std::unexpected(make_status(status_code::resource_exhausted,
                                         "grpc message exceeds send limit"));
    }
  }
  return {};
}
...
client::client(io_context &ctx, std::string base_url, http::client_options opts)
    : client(ctx, std::move(base_url),
             client_options{.http = std::move(opts)}) {}

client::client(io_context &ctx, std::string base_url, client_options opts)
    : http_(ctx, std::move(opts.http)), base_url_(std::move(base_url)),
      opts_(std::move(opts)) {
...
auto client::send_streaming(streaming_request req)
  ...
  if (req.compression != compression_algorithm::identity) {
    http_req.set_header("grpc-encoding",
                        std::string(compression_name(req.compression)));
  }
  ...
  if (!out.st.ok())
    co_return std::unexpected(out.st);
  co_return out;
}
...
auto client::unary(unary_request req)
  ...
  if (req.compression == compression_algorithm::identity &&
    ...
    if (metadata_wire_size(req.headers) > opts_.max_metadata_bytes) {
      co_return std::unexpected(make_status(
          status_code::resource_exhausted, "grpc metadata exceeds send limit"));
    }
    if (req.payload.size() > opts_.max_send_message_bytes) {
      co_return std::unexpected(make_status(status_code::resource_exhausted,
                                            "grpc message exceeds send limit"));
    }
...
auto client::client_streaming(streaming_request req)
  ...
  if (!r)
    co_return std::unexpected(r.error());
  if (r->messages.size() > 1) {
    co_return std::unexpected(make_internal(
        "client-streaming grpc response returned multiple messages"));
  }
  unary_response out{
      .st = std::move(r->st),
      .headers = std::move(r->headers),
