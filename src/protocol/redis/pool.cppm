...
// =============================================================================
// Connection node state — P0: Each node has its own lifecycle task
// =============================================================================
    ...
    auto connection_task(conn_node& node) -> task<void> {
        ...
        while (running_) {
            ...
            if (state == conn_state::idle) {
                ...
                auto rs = co_await node.conn->cmd({"PING"});
...
//   - Thread-local shard selection via round-robin or io_context binding
