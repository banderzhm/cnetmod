...
void add_header(std::string &payload, resp3_type t, std::size_t size);
...
void serialize_one(std::string &payload, std::string_view sv);

void serialize_one(std::string &payload, const std::string &s);

void serialize_one(std::string &payload, const char *s);
...
template <class T> constexpr auto bulk_count() -> std::size_t { return 1; }
...
/// Composed of one or more Redis commands, encoded using RESP3 array of bulk
/// strings
///
/// Usage:
///   request req;
...
public:
  ...
  template <class ForwardIterator>
  void push_range(std::string_view cmd, ForwardIterator begin,
                  ForwardIterator end) {
    if (begin == end)
      return;
    auto distance = static_cast<std::size_t>(std::distance(begin, end));
