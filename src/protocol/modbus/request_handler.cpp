...
class request_handler_impl {
public:
  explicit request_handler_impl(data_store &store) : store_(store) {}
  ...
  auto handle(const modbus_request &request) -> modbus_response {
    ...
    try {
      ...
      default:
        return make_exception_response(request,
...
private:
  ...
  auto handle_read_coils(const modbus_request &request) -> modbus_response {
    ...
    for (std::size_t i = 0; i < byte_count; ++i) {
      ...
      for (std::size_t bit = 0; bit < 8 && (i * 8 + bit) < quantity; ++bit) {
        auto result = store_.read_coil(start_address +
                                       static_cast<std::uint16_t>(i * 8 + bit));
        if (!result) {
  ...
  auto handle_read_discrete_inputs(const modbus_request &request)
      -> modbus_response {
    if (request.data.size() < 4) {
      return make_exception_response(request,
                                     exception_code::illegal_data_value);
    ...
    for (std::size_t i = 0; i < byte_count; ++i) {
      std::uint8_t byte = 0;
      for (std::size_t bit = 0; bit < 8 && (i * 8 + bit) < quantity; ++bit) {
        auto result = store_.read_discrete_input(
            start_address + static_cast<std::uint16_t>(i * 8 + bit));
        if (!result) {
          return make_exception_response(request, result.error());
        }
        if (*result) {
  ...
  auto handle_read_holding_registers(const modbus_request &request)
      -> modbus_response {
    if (request.data.size() < 4) {
      return make_exception_response(request,
                                     exception_code::illegal_data_value);
  ...
  auto handle_read_input_registers(const modbus_request &request)
      -> modbus_response {
    if (request.data.size() < 4) {
      return make_exception_response(request,
                                     exception_code::illegal_data_value);
  ...
  auto handle_write_single_coil(const modbus_request &request)
      -> modbus_response {
    if (request.data.size() < 4) {
      return make_exception_response(request,
                                     exception_code::illegal_data_value);
  ...
  auto handle_write_single_register(const modbus_request &request)
      -> modbus_response {
    if (request.data.size() < 4) {
      return make_exception_response(request,
                                     exception_code::illegal_data_value);
  ...
  auto handle_write_multiple_coils(const modbus_request &request)
      -> modbus_response {
  ...
  auto handle_write_multiple_registers(const modbus_request &request)
      -> modbus_response {
    if (request.data.size() < 5) {
      return make_exception_response(request,
                                     exception_code::illegal_data_value);
...
class request_handler::impl final : public request_handler_impl {
public:
  using request_handler_impl::request_handler_impl;
};

request_handler::request_handler(data_store &store)
    : impl_(std::make_unique<impl>(store)) {}
request_handler::~request_handler() = default;
request_handler::request_handler(request_handler &&) noexcept = default;
auto request_handler::operator=(request_handler &&) noexcept
    -> request_handler & = default;
auto request_handler::handle(const modbus_request &request) -> modbus_response {
