...
public:
  explicit udp_client_impl(io_context &ctx) : ctx_(ctx), transaction_id_(0) {}

  udp_client_impl(const udp_client_impl &) = delete;
  auto operator=(const udp_client_impl &) -> udp_client_impl & = delete;
  ...
  auto connect(std::string_view host, std::uint16_t port)
    ...
    auto addrs =
        co_await async_resolve_addresses(ctx_, host, std::to_string(port));
    ...
    endpoint local_ep(addr_result.is_v4() ? ip_address(ipv4_address::any())
  ...
  auto execute(const modbus_request &request)
    ...
    // Send request
    const_buffer send_buf{reinterpret_cast<const std::byte *>(data.data()),
                          data.size()};
    auto send_result =
        co_await async_sendto(ctx_, socket_, send_buf, remote_endpoint_);
    if (!send_result) {
    ...
    endpoint from_ep;
    mutable_buffer recv_buf{reinterpret_cast<std::byte *>(buffer.data()),
                            buffer.size()};
    auto recv_result =
        co_await async_recvfrom(ctx_, socket_, recv_buf, from_ep);
    if (!recv_result) {
      co_return std::unexpected(recv_result.error());
    }

    // Validate source (optional - UDP can receive from anywhere)
    // For strict Modbus, you might want to check from_ep matches
    // remote_endpoint_
  ...
  // ── Check if open ──
  auto is_open() const -> bool { return socket_.is_open(); }

  // ── Get next transaction ID ──
  auto next_transaction_id() -> std::uint16_t { return transaction_id_++; }
...
class udp_client::impl final : public udp_client_impl {
public:
  using udp_client_impl::udp_client_impl;
};
udp_client::udp_client(io_context &ctx) : impl_(std::make_unique<impl>(ctx)) {}
udp_client::~udp_client() = default;
auto udp_client::connect(std::string_view host, std::uint16_t port)
    -> task<std::error_code> {
  return impl_->connect(host, port);
}
auto udp_client::execute(const modbus_request &request)
    -> task<std::expected<modbus_response, std::error_code>> {
  return impl_->execute(request);
}
auto udp_client::execute_with_retry(const modbus_request &request, int retries)
    -> task<std::expected<modbus_response, std::error_code>> {
  return impl_->execute_with_retry(request, retries);
}
void udp_client::close() { impl_->close(); }
auto udp_client::is_open() const -> bool { return impl_->is_open(); }
auto udp_client::next_transaction_id() -> std::uint16_t {
  return impl_->next_transaction_id();
}
auto udp_client::get_remote_host() const -> const std::string & {
  return impl_->get_remote_host();
}
auto udp_client::get_remote_port() const -> std::uint16_t {
