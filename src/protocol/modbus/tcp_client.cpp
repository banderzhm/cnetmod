...
public:
  explicit tcp_client_impl(io_context &ctx) : ctx_(ctx), transaction_id_(0) {}

  tcp_client_impl(const tcp_client_impl &) = delete;
  auto operator=(const tcp_client_impl &) -> tcp_client_impl & = delete;
  ...
  auto execute(const modbus_request &request)
    ...
    mutable_buffer header_mbuf{reinterpret_cast<std::byte *>(header_buf.data()),
                               7};
  ...
  auto execute_with_timeout(const modbus_request &request,
      ...
      -> task<std::expected<modbus_response, std::error_code>> {
    cancel_token token;
    co_return co_await with_timeout(ctx_, timeout,
                                    execute_with_cancel(request, token), token);
  }
  ...
  auto read_exact(mutable_buffer buf, cancel_token &token)
    ...
    while (got < buf.size) {
      auto r = co_await async_read(
          ctx_, socket_, mutable_buffer{data + got, buf.size - got}, token);
      if (!r) {
        co_return std::unexpected(r.error());
      }
      if (*r == 0) {
...
class tcp_client::impl final : public tcp_client_impl {
public:
  using tcp_client_impl::tcp_client_impl;
};
tcp_client::tcp_client(io_context &ctx) : impl_(std::make_unique<impl>(ctx)) {}
tcp_client::~tcp_client() = default;
auto tcp_client::connect(std::string_view host, std::uint16_t port)
    -> task<std::error_code> {
  return impl_->connect(host, port);
}
auto tcp_client::execute(const modbus_request &request)
    -> task<std::expected<modbus_response, std::error_code>> {
  return impl_->execute(request);
}
auto tcp_client::execute_with_timeout(
    const modbus_request &request, std::chrono::steady_clock::duration timeout)
    -> task<std::expected<modbus_response, std::error_code>> {
  return impl_->execute_with_timeout(request, timeout);
}
auto tcp_client::reconnect() -> task<std::error_code> {
  return impl_->reconnect();
}
void tcp_client::close() { impl_->close(); }
auto tcp_client::is_open() const -> bool { return impl_->is_open(); }
auto tcp_client::next_transaction_id() -> std::uint16_t {
  return impl_->next_transaction_id();
}
auto tcp_client::get_host() const -> const std::string & {
  return impl_->get_host();
}
auto tcp_client::get_port() const -> std::uint16_t { return impl_->get_port(); }
