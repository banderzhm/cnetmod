...
auto tcp_server::get_host() const -> const std::string&
{
    return host_;
}

auto tcp_server::get_port() const -> std::uint16_t
{
    return port_;
}
    ...
    while (client.is_open() && running_)
    {
        ...
        if (length > 1)
        {
            const auto result = co_await read_exact(
                client, {reinterpret_cast<std::byte*>(payload.data() + 7), static_cast<std::size_t>(length - 1)});
            if (!result)
                break;
        }
