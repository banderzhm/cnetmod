...
struct rtu_server_config_implementation_only {
...
class rtu_server_impl {
public:
  rtu_server_impl(io_context &ctx, data_store &store)
      : ctx_(ctx), handler_(store) {}

  rtu_server_impl(const rtu_server_impl &) = delete;
  auto operator=(const rtu_server_impl &) -> rtu_server_impl & = delete;
  ...
  // ── Check if running ──
  auto is_running() const -> bool { return running_ && serial_.is_open(); }

  // ── Get configuration ──
  auto get_config() const -> const rtu_server_config & { return config_; }
...
private:
  ...
  auto receive_loop() -> task<void> {
    ...
    while (running_) {
      ...
      if (request.header.unit_id != config_.unit_id &&
          request.header.unit_id != 0) {
        buffer.resize(256);
...
auto rtu_server_config::to_serial_config() const -> serial_config {
  serial_config cfg;
  cfg.baud_rate = baudrate;
  cfg.data_bits = data_bits;
  cfg.stop = stop;
...
class rtu_server::impl final : public rtu_server_impl {
public:
  using rtu_server_impl::rtu_server_impl;
};
rtu_server::rtu_server(io_context &ctx, data_store &store)
    : impl_(std::make_unique<impl>(ctx, store)) {}
rtu_server::~rtu_server() = default;
auto rtu_server::start(const rtu_server_config &c) -> task<std::error_code> {
  return impl_->start(c);
}
void rtu_server::stop() { impl_->stop(); }
auto rtu_server::is_running() const -> bool { return impl_->is_running(); }
auto rtu_server::get_config() const -> const rtu_server_config & {
