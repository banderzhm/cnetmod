...
public:
    ...
    auto& get_discrete_inputs()
    {
        return discrete_inputs_;
    }

    auto& get_holding_registers()
    {
        return holding_registers_;
    }

    auto& get_input_registers()
    {
...
public:
    channel_data_store_impl(std::size_t coil_count = 10000,
        ...
        : req_channel_(channel_capacity), resp_channel_(channel_capacity), coils_(coil_count, false), discrete_inputs_(discrete_input_count, false), holding_registers_(holding_register_count, 0), input_registers_(input_register_count, 0) {}
...
void channel_data_store::start_worker()
{
    impl_->start_worker();
}

void channel_data_store::stop_worker()
{
    impl_->stop_worker();
}

auto channel_data_store::worker() -> task<void>
{
    return impl_->worker();
}
