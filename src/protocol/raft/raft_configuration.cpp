...
namespace {
    ...
    {
        std::erase_if(ids, [](const node_id& id)
            {
                return id.empty();
            });
        std::ranges::sort(ids);
    ...
    auto majority_of(std::size_t n) noexcept -> std::size_t
    {
        return n / 2 + 1;
    }
} // namespace
...
auto configuration::joint() const noexcept -> bool
{
    return state_.joint();
}
...
vote_tracker::vote_tracker(configuration conf)
    : conf_(std::move(conf)) {}
