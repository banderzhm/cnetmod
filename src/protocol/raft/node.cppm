...
public:
  ...
  [[nodiscard]] auto
  election_timeout_elapsed(std::chrono::steady_clock::duration timeout) const
      -> bool;
  [[nodiscard]] auto
  leader_lease_valid(std::chrono::steady_clock::duration timeout) const -> bool;
  ...
  auto handle_request_vote(const request_vote_request &req)
  ...
  auto handle_append_entries_response(const node_id &peer,
                                      const append_entries_response &resp)
      -> bool;
  ...
  auto handle_install_snapshot_response(const node_id &peer,
                                        const install_snapshot_response &resp)
      -> bool;

  auto append_command(std::string command)
  ...
  auto set_learners(std::vector<node_id> learners)
      -> std::expected<log_entry, raft_error>;
  auto promote_learner(const node_id &id)
      -> std::expected<log_entry, raft_error>;
  ...
  auto create_snapshot(std::string uri)
  ...
  auto query_read_index(request_id id) const
      -> std::optional<read_index_response>;
  auto consume_read_index(request_id id) -> std::optional<read_index_response>;
  auto expire_read_indexes(std::chrono::steady_clock::duration timeout)
      -> std::size_t;
...
private:
  ...
  [[nodiscard]] auto make_timeout_now(const node_id &target) const
      -> std::optional<timeout_now_request>;
  auto append_configuration(configuration next)
      -> std::expected<log_entry, raft_error>;
