...
{
    if (!store_)
        throw std::invalid_argument("raft_node requires storage");
...
void raft_node::apply_committed()
{
    ...
    {
        if (entry.index != state_.last_applied + 1)
            return false;
        ...
        if (entry.type == entry_type::configuration)
        {
            ...
            else if (role_ == node_role::leader && conf_.joint())
            {
                ...
                if (next)
                {
                    ...
                    {
                        stop({.code = raft_errc::storage_error,
    ...
    while (state_.last_applied < state_.commit_index)
    {
        ...
        else
        {
            ...
            if (entries.empty() || entries.front().index != first)
                return;
