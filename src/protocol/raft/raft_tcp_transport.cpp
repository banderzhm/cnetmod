...
inline auto crc32(std::span<const std::byte> data,
                  std::uint32_t crc = 0xffffffffu) -> std::uint32_t {
  for (auto byte : data) {
...
inline auto crc32_bytes(std::span<const std::byte> data) noexcept
    -> std::uint32_t {
...
inline auto constant_time_equal(std::string_view left,
...
template <typename F> class thread_invoke_awaitable {
public:
  ...
  void await_suspend(std::coroutine_handle<> continuation) noexcept {
    ...
    try {
      std::thread([st = std::move(st)]() mutable {
        ...
        (*posted_state)
            ->ctx->post(&resume_on_context, posted_state, [](void *p) {
...
template <typename F> auto thread_invoke(io_context &ctx, F &&fn) {
...
inline auto write_file_all(io_context &ctx, file &out,
                           std::span<const std::byte> data,
                           std::uint64_t offset)
    -> task<std::expected<void, std::error_code>> {
  std::size_t written = 0;
  while (written < data.size()) {
    auto r = co_await async_file_write(
        ctx, out, const_buffer{data.data() + written, data.size() - written},
        offset + written);
    if (!r)
...
inline auto crc32_file(io_context &ctx, const std::filesystem::path &path)
  ...
  if (!stat)
    co_return std::unexpected(stat.error());
  ...
  while (offset < stat->size) {
    const auto want =
        std::min<std::uint64_t>(buffer.size(), stat->size - offset);
    auto r = co_await async_file_read(
        ctx, input,
        mutable_buffer{buffer.data(), static_cast<std::size_t>(want)}, offset);
    if (!r) {
  ...
  if (!close)
    co_return std::unexpected(close.error());
...
inline auto read_exact(io_context &ctx, socket &sock, std::span<std::byte> data)
  ...
  while (done < data.size()) {
    auto r = co_await async_read(
        ctx, sock, mutable_buffer{data.data() + done, data.size() - done});
    if (!r)
      co_return std::unexpected(r.error());
    if (*r == 0)
...
inline auto read_message(io_context &ctx, socket &sock)
  ...
  try {
    payload_size =
        raft_frame_payload_size(std::span<const std::byte, 9>{header});
  } catch (...) {
  ...
  if (payload_size != 0) {
    if (auto r = co_await read_exact(
            ctx, sock, std::span<std::byte>{frame}.subspan(header.size()));
        !r)
      co_return std::unexpected(r.error());
  }
...
inline auto read_exact(ssl_stream &stream, std::span<std::byte> data)
  ...
  std::size_t done = 0;
  while (done < data.size()) {
    auto r = co_await stream.async_read(
        mutable_buffer{data.data() + done, data.size() - done});
    if (!r)
      co_return std::unexpected(r.error());
    if (*r == 0)
      co_return std::unexpected(
...
inline auto read_message(ssl_stream &stream)
  ...
  if (payload_size != 0) {
    if (auto r = co_await read_exact(
            stream, std::span<std::byte>{frame}.subspan(header.size()));
        !r)
      co_return std::unexpected(r.error());
  }
...
inline auto send_raw(io_context &ctx, const endpoint &ep,
...
raft_tcp_transport::inbound_socket_guard::inbound_socket_guard(
    raft_tcp_transport &transport, socket &inbound,
...
raft_tcp_transport::outbound_tls_socket_guard::outbound_tls_socket_guard(
    raft_tcp_transport &transport, socket &outbound) noexcept
...
raft_tcp_transport::raft_tcp_transport(io_context &ctx, node_id local_id,
                                       raft_tcp_transport_options options)
    : ctx_(ctx), local_id_(std::move(local_id)), options_(options) {}

void raft_tcp_transport::set_node(raft_node &node) noexcept { node_ = &node; }

void raft_tcp_transport::set_error_handler(
    std::function<void(node_id, std::error_code)> handler) {

  on_error_ = std::move(handler);
}

void raft_tcp_transport::add_peer(raft_tcp_peer peer) {
...
void raft_tcp_transport::remove_peer(const node_id &peer) {
...
[[nodiscard]] auto raft_tcp_transport::peers() const -> std::vector<node_id> {
...
[[nodiscard]] auto raft_tcp_transport::peer_metrics() const
...
[[nodiscard]] auto raft_tcp_transport::peer_metrics(const node_id &peer) const
...
void raft_tcp_transport::send_pre_vote(const node_id &peer,
...
void raft_tcp_transport::send_request_vote(
    const node_id &peer, const request_vote_request &request) {
...
void raft_tcp_transport::send_request_vote_response(
    const node_id &peer, const request_vote_response &response) {
...
void raft_tcp_transport::send_timeout_now(const node_id &peer,
...
void raft_tcp_transport::send_append_entries(
    const node_id &peer, const append_entries_request &request) {
...
void raft_tcp_transport::send_append_entries_response(
    const node_id &peer, const append_entries_response &response) {
...
void raft_tcp_transport::send_install_snapshot(
    const node_id &peer, const install_snapshot_request &request) {
  ...
  if (!request.uri.empty() && request.data.empty()) {
    ...
    spawn_tracked(send_snapshot_file(peer, it->second, request));
    return;
  }
  if (!request.data.empty() && request.data.size() > request_chunk_size()) {
    ...
    spawn_tracked(send_snapshot_memory(peer, it->second, request));
    return;
  }
...
void raft_tcp_transport::send_install_snapshot_response(
    const node_id &peer, const install_snapshot_response &response) {
...
void raft_tcp_transport::broadcast_pre_vote(raft_node &node) {

  auto req = node.begin_pre_vote();
  for (const auto &peer : peers())
    send_pre_vote(peer, req);
}

void raft_tcp_transport::broadcast_request_vote(raft_node &node) {

  auto req = node.begin_election();
  for (const auto &peer : peers())
    send_request_vote(peer, req);
}

void raft_tcp_transport::replicate_to_all(raft_node &node) {
  ...
  for (const auto &peer : peers()) {
    if (node.should_send_snapshot(peer)) {
      ...
      const auto last_sent =
          req.entries.empty() ? req.prev_log_index : req.entries.back().index;
...
auto raft_tcp_transport::transfer_leader(raft_node &node, const node_id &target)
...
auto raft_tcp_transport::serve(endpoint listen_endpoint)
    -> task<std::expected<void, std::error_code>> {
...
auto raft_tcp_transport::serve(endpoint listen_endpoint, cancel_token &token)
    -> task<std::expected<void, std::error_code>> {
  ...
  while (running_ && !token.is_cancelled()) {
    token.reset();
    auto accepted =
        co_await async_accept(ctx_, acceptor.native_socket(), token);
    if (!accepted) {
      if (token.is_cancelled())
        co_return {};
      co_return std::unexpected(accepted.error());
    }
    spawn_tracked(handle_connection(std::move(*accepted)));
  }
  co_return {};
}

void raft_tcp_transport::stop() noexcept {
...
auto raft_tcp_transport::async_stop() -> task<void> {
  stop();
  while (active_tasks_.load(std::memory_order_acquire) != 0)
    (void)co_await async_timer_wait(ctx_, std::chrono::milliseconds{1});
}

void raft_tcp_transport::spawn_tracked(task<void> operation) {
  active_tasks_.fetch_add(1, std::memory_order_acq_rel);
  spawn(ctx_, run_tracked(std::move(operation)));
}

auto raft_tcp_transport::run_tracked(task<void> operation) -> task<void> {
...
auto raft_tcp_transport::cleanup_snapshot_sessions() -> std::size_t {

  return cleanup_snapshot_sessions(options_.snapshot_session_timeout);
}

auto raft_tcp_transport::cleanup_snapshot_sessions(
    std::chrono::steady_clock::duration timeout) -> std::size_t {
...
auto raft_tcp_transport::cleanup_snapshot_files()
    -> task<std::expected<std::size_t, std::error_code>> {

  auto directory = options_.snapshot_directory;
  auto retention = options_.snapshot_retention;
  co_return co_await tcp_transport_detail::thread_invoke(
      ctx_, [directory = std::move(directory), retention] {
        return cleanup_snapshot_files_blocking(directory, retention);
      });
}

void raft_tcp_transport::send(const node_id &peer, raft_rpc_message message) {
  ...
  metrics.max_queue_depth =
      std::max<std::uint64_t>(metrics.max_queue_depth,
                              static_cast<std::uint64_t>(conn.outbound.size()));

  if (!conn.writer_running) {
    conn.writer_running = true;
    spawn_tracked(send_writer(peer));
  }
}

void raft_tcp_transport::notify_error(const node_id &peer, std::error_code ec) {

  if (on_error_)
    on_error_(peer, ec);
}

auto raft_tcp_transport::send_writer(node_id peer) -> task<void> {

  for (;;) {
    if (!running_) {
      if (auto conn_it = connections_.find(peer);
    ...
    auto r =
        co_await send_with_retry(peer, item.ep, item.message, conn.write_token);
...
void raft_tcp_transport::coalesce_append_entries(
    const node_id &peer, peer_connection &conn,
...
[[nodiscard]] auto raft_tcp_transport::droppable_append_entries(
    const append_entries_request &request) -> bool {

  return request.entries.empty() && request.read_contexts.empty();
}

auto raft_tcp_transport::send_with_retry(const node_id &peer, endpoint ep,
                                         const raft_rpc_message &message,
                                         cancel_token &token)
    -> task<std::expected<void, std::error_code>> {
...
auto raft_tcp_transport::handle_connection(socket sock) -> task<void> {
  ...
  auto message =
      co_await tcp_transport_detail::read_message(ctx_, sock, read_token);
  while (message && node_) {
    co_await dispatch_message(*message);
    read_token.reset();
    message =
...
auto raft_tcp_transport::send_tls_once(const node_id &peer, const endpoint &ep,
                                       const raft_rpc_message &message,
                                       cancel_token &token)
    -> task<std::expected<void, std::error_code>> {
  ...
  auto sent =
      co_await stream.async_write_all(const_buffer{frame.data(), frame.size()});
  (void)co_await stream.async_shutdown();
  sock.close();
  co_return sent;
#else
...
#ifdef CNETMOD_HAS_SSL
[[nodiscard]] auto
raft_tcp_transport::verify_tls_peer(ssl_stream &stream,
                                    const node_id &peer) const
    -> std::expected<void, std::error_code> {

  auto it = options_.security.peer_certificate_sha256.find(peer);
  if (it == options_.security.peer_certificate_sha256.end() ||
      it->second.empty()) {
...
auto raft_tcp_transport::send_pooled(const node_id &peer, const endpoint &ep,
                                     const raft_rpc_message &message)
    -> task<std::expected<void, std::error_code>> {
  ...
  if (!conn.sock.is_open()) {
    ...
    conn.write_token.reset();
    if (auto r = co_await async_connect(ctx_, conn.sock, ep, conn.write_token);
        !r) {
      conn.sock.close();
      co_return std::unexpected(r.error());
    }
  ...
  conn.write_token.reset();
  auto r = co_await async_write_all(ctx_, conn.sock,
                                    const_buffer{frame.data(), frame.size()},
                                    conn.write_token);
  if (!r)
    conn.sock.close();
...
auto raft_tcp_transport::dispatch_message(raft_rpc_message &message)
    -> task<void> {
  ...
  case raft_rpc_type::request_vote_response: {
    const bool quorum =
        node_->handle_vote_response(message.from, message.vote_response);
  ...
  case raft_rpc_type::append_entries_response: {
    const bool advanced = node_->handle_append_entries_response(
        message.from, message.append_response);
  ...
  case raft_rpc_type::install_snapshot: {
    ...
    if (status == snapshot_materialize_status::complete) {
      ...
      resp = install_snapshot_response{
          ...
          .accepted_offset =
              accepted_snapshot_offset(message.from, message.snapshot),
  ...
  case raft_rpc_type::install_snapshot_response:
    ...
    if (node_->handle_install_snapshot_response(message.from,
      ...
      snapshot_sends_.erase(snapshot_send_key(
          message.from, message.snapshot_response.snapshot_id));
      if (auto transfer = node_->take_pending_leader_transfer(message.from))
        send_timeout_now(message.from, *transfer);
      else
        replicate_to_all(*node_);
...
auto raft_tcp_transport::materialize_snapshot(raft_rpc_message &message)
  ...
  if (message.snapshot.offset < state.expected_offset) {
    if (message.snapshot.offset + message.snapshot.data.size() <=
        state.expected_offset)
      co_return message.snapshot.done &&state.expected_offset ==
              message.snapshot.total_size
  ...
  if (!out)
    co_return snapshot_materialize_status::failed;
  auto written = co_await tcp_transport_detail::write_file_all(
      ctx_, *out, std::span<const std::byte>{message.snapshot.data},
      state.expected_offset);
  if (!written) {
...
auto raft_tcp_transport::send_snapshot_file(node_id peer, endpoint ep,
  ...
  request.snapshot_id = request.snapshot_id.empty()
                            ...
                            : request.snapshot_id;
  if (start_offset == 0) {
    snapshot_sends_[snapshot_send_key(peer, request.snapshot_id)] =
        snapshot_send_state{
            .ep = ep,
            .request = request,
  ...
  do {
    ...
    while (got < want) {
      auto r = co_await async_file_read(
          ctx_, input,
          mutable_buffer{buffer.data() + got,
    ...
    if (auto it =
            snapshot_sends_.find(snapshot_send_key(peer, request.snapshot_id));
...
auto raft_tcp_transport::send_snapshot_memory(node_id peer, endpoint ep,
                                              install_snapshot_request request)
    -> task<void> {
  ...
  for (std::uint64_t offset =
           std::min<std::uint64_t>(request.offset, payload.size());
       offset < payload.size(); offset += chunk_size) {
    ...
    chunk.data.assign(payload.begin() + static_cast<std::ptrdiff_t>(offset),
                      payload.begin() +
                          static_cast<std::ptrdiff_t>(offset + got));
    chunk.chunk_crc32 = tcp_transport_detail::crc32_bytes(chunk.data);
...
auto raft_tcp_transport::resume_snapshot_send(
    const node_id &peer, const install_snapshot_response &response)
    -> task<void> {
...
[[nodiscard]] auto raft_tcp_transport::request_chunk_size() const noexcept
    -> std::size_t {

  return std::max<std::size_t>(1, options_.snapshot_chunk_size);
}

[[nodiscard]] auto raft_tcp_transport::default_snapshot_id(
    const install_snapshot_request &snapshot) const -> std::string {
...
[[nodiscard]] auto raft_tcp_transport::snapshot_path(
    const node_id &from, const install_snapshot_request &snapshot) const
    -> std::filesystem::path {

  const auto name = std::format(
      "{}-{}-{}-{}.snapshot", from, snapshot.metadata.last_included_index,
      snapshot.metadata.last_included_term, snapshot.snapshot_id);
  return options_.snapshot_directory / name;
}

[[nodiscard]] auto raft_tcp_transport::snapshot_session_key(
    const node_id &from, const install_snapshot_request &snapshot)
    -> std::string {

  return std::format("{}:{}", from, snapshot.snapshot_id);
}

[[nodiscard]] auto raft_tcp_transport::snapshot_send_key(
    const node_id &peer, std::string_view snapshot_id) -> std::string {

  return std::format("{}:{}", peer, snapshot_id);
}

[[nodiscard]] auto raft_tcp_transport::accepted_snapshot_offset(
    const node_id &from, const install_snapshot_request &snapshot) const
    -> std::uint64_t {
...
[[nodiscard]] auto raft_tcp_transport::is_chunked_snapshot(
    const install_snapshot_request &snapshot) -> bool {

  return !snapshot.data.empty() || !snapshot.snapshot_id.empty() ||
         snapshot.total_size != 0 || snapshot.offset != 0 ||
         snapshot.chunk_crc32 != 0 || snapshot.file_crc32 != 0 ||
         !snapshot.done;
}

auto raft_tcp_transport::cleanup_snapshot_files_blocking(
  ...
  std::ranges::sort(files, std::greater<>{},
                    &snapshot_file_candidate::modified);
...
[[nodiscard]] auto raft_tcp_transport::fnv1a64(std::string_view data,
                                               std::uint64_t seed)
    -> std::uint64_t {
...
[[nodiscard]] auto
raft_tcp_transport::make_auth_token(const raft_rpc_message &message) const
    -> std::string {
...
[[nodiscard]] auto
raft_tcp_transport::authenticate(const raft_rpc_message &message) const
    -> bool {
...
auto raft_tcp_transport::metrics_for(const node_id &peer)
    -> raft_peer_transport_metrics & {
...
void raft_tcp_transport::record_send_success(
    const node_id &peer, std::chrono::steady_clock::duration latency) {
...
void raft_tcp_transport::record_send_failure(
    const node_id &peer, std::error_code ec,
