...
fsm_caller::fsm_caller(state_machine* machine)
    : machine_(machine) {}

void fsm_caller::reset(state_machine* machine)
{
    machine_ = machine;
}
    ...
    -> std::expected<log_index, raft_error>
{
    ...
    if (machine_ && (entry.type == entry_type::command || entry.type == entry_type::configuration))
    {
        try
        {
            machine_->on_apply(entry);
