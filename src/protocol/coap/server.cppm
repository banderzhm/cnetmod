/// cnetmod.protocol.coap:server - UDP CoAP server and resource router interface.
...
public:
    ...
    [[nodiscard]] auto dispatch(const inbound_request& req, const endpoint& peer) const
        -> task<message>;
...
private:
    ...
    static auto has_unsupported_critical_option(const message& msg) noexcept -> bool;
    static auto response_content_format(const message& resp) -> std::optional<std::uint32_t>;
    static auto request_accepts_response(const inbound_request& req, const message& resp) -> bool;
    auto check_preconditions(const inbound_request& req) const -> std::optional<message>;

    auto make_discovery_response(const inbound_request& req) -> std::optional<message>;
    auto parse_proxy_uri(std::string_view uri) -> std::optional<proxy_target>;
    auto proxy_request(const inbound_request& req) -> task<std::optional<message>>;

    auto apply_block1(const endpoint& peer, inbound_request& req) -> block1_result;
    void apply_block2(const inbound_request& req, message& resp);

    void apply_observe_state(const inbound_request& req, const endpoint& peer, message& resp);
    void add_observer(const endpoint& peer, const std::vector<std::byte>& token, std::string path);
    void remove_observer(const endpoint& peer, const std::vector<std::byte>& token, std::string path);
    auto collect_observers(std::string path) -> std::vector<observe_subscription>;
    void track_observe_delivery(std::uint16_t message_id, const observe_subscription& sub);
    auto handle_observe_control(const endpoint& peer, const message& msg) -> bool;
    static auto observe_delivery_key(const endpoint& peer, std::uint16_t message_id) -> std::string;
    ...
    std::unordered_map<observer_key, observe_subscription, observer_hash> observers_;
