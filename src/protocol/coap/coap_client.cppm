...
public:
  ...
  auto
  observe(const endpoint &remote, std::string path, observe_handler handler,
  ...
  auto post(const endpoint &remote, std::string path,
            std::vector<std::byte> payload,
            content_format format = content_format::octet_stream)
      -> task<std::expected<message, std::error_code>>;

  auto post_blockwise(const endpoint &remote, std::string path,
                      std::vector<std::byte> payload,
                      content_format format = content_format::octet_stream,
                      std::uint8_t size_exponent = 6)
      -> task<std::expected<message, std::error_code>>;

  auto put(const endpoint &remote, std::string path,
           std::vector<std::byte> payload,
           content_format format = content_format::octet_stream)
      -> task<std::expected<message, std::error_code>>;

  auto put_blockwise(const endpoint &remote, std::string path,
                     std::vector<std::byte> payload,
                     content_format format = content_format::octet_stream,
                     std::uint8_t size_exponent = 6)
      -> task<std::expected<message, std::error_code>>;
  ...
  auto resolve_endpoint(std::string_view host,
...
private:
  ...
  auto receive_notification(const endpoint &remote,
  ...
  auto upload_blockwise(const endpoint &remote, method method_code,
