...
auto message::is_response() const noexcept -> bool { return code >= 64; }
...
void message::add_option(std::uint16_t number,
                         std::span<const std::byte> bytes) {
  options.push_back(option{number, {bytes.begin(), bytes.end()}});
}

void message::add_option(option_number number,
                         std::span<const std::byte> bytes) {
...
auto message::first_option(option_number number) const
    -> std::optional<option> {
  const auto n = static_cast<std::uint16_t>(number);
  for (const auto &opt : options) {
    if (opt.number == n) {
