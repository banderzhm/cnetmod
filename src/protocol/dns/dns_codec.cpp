...
module cnetmod.protocol.dns.codec;
...
inline auto read_u32(std::span<const std::byte> data, std::size_t pos)
  ...
  if (pos + 4 > data.size())
    return std::nullopt;
  return (static_cast<std::uint32_t>(std::to_integer<unsigned char>(data[pos]))
          << 24) |
         (static_cast<std::uint32_t>(
              std::to_integer<unsigned char>(data[pos + 1]))
          << 16) |
         (static_cast<std::uint32_t>(
              std::to_integer<unsigned char>(data[pos + 2]))
          << 8) |
         static_cast<std::uint32_t>(
             std::to_integer<unsigned char>(data[pos + 3]));
}
...
inline auto write_name(std::vector<std::byte> &out, std::string_view name)
    -> bool {
  if (name.empty() || name == ".") {
  ...
  while (!name.empty()) {
    ...
    out.insert(
        out.end(), reinterpret_cast<const std::byte *>(label.data()),
...
inline auto write_question(std::vector<std::byte> &out, const question &q)
    -> bool {
  if (!write_name(out, q.name))
    return false;
...
inline auto write_rr(std::vector<std::byte> &out, const resource_record &rr)
    -> bool {
  if (!write_name(out, rr.name))
    return false;
  put_u16(out, static_cast<std::uint16_t>(rr.type));
  put_u16(out, static_cast<std::uint16_t>(rr.cls));
  put_u32(out, rr.ttl);
  if (rr.data.size() > std::numeric_limits<std::uint16_t>::max())
    return false;
...
auto make_query(std::string_view name, record_type type, std::uint16_t id)
    -> message {
  if (id == 0) {
...
auto a_record(std::string_view name, const ipv4_address &addr,
              std::uint32_t ttl) -> resource_record {
...
auto txt_record(std::string_view name, std::string_view text, std::uint32_t ttl)
  ...
  data.insert(data.end(), reinterpret_cast<const std::byte *>(text.data()),
