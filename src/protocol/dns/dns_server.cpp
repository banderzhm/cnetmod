...
namespace {
        ...
        -> task<std::expected<message, std::error_code>>
    {
        ...
        if (!lr)
            co_return std::unexpected(lr.error());
        ...
        if (!br)
            co_return std::unexpected(br.error());
...
udp_server::udp_server(io_context& ctx)
    : ctx_(ctx), sock_(ctx) {}
...
void udp_server::set_handler(query_handler handler)
{
...
tcp_server::tcp_server(io_context& ctx)
    : ctx_(ctx), acc_(ctx) {}
...
void tcp_server::set_handler(query_handler handler)
{
    handler_ = std::move(handler);
}
    ...
    -> std::expected<void, std::error_code>
{
    ...
    if (!ctx_r)
        return std::unexpected(ctx_r.error());
    ssl_ctx_ = std::make_unique<ssl_context>(std::move(*ctx_r));
    if (auto r = ssl_ctx_->load_cert_file(opts_.cert_file); !r)
        return std::unexpected(r.error());
    if (auto r = ssl_ctx_->load_key_file(opts_.key_file); !r)
        return std::unexpected(r.error());
...
void dot_server::set_handler(query_handler handler)
{
    handler_ = std::move(handler);
}

void dot_server::stop() noexcept
{
    running_ = false;
    acc_.close();
}
