...
auto parse_server_greeting(const std::uint8_t *data, std::size_t size)
  ...
  if (size < 4)
    return std::unexpected("greeting too short");
  ...
  if (sv.bytes_consumed == 0)
    return std::unexpected("bad server version");
  g.server_version = std::string(sv.value);
  pos += sv.bytes_consumed;

  if (pos + 4 > size)
    return std::unexpected("greeting truncated (conn_id)");
  g.connection_id = read_u32_le(data + pos);
  pos += 4;

  // auth_plugin_data part1 (8 bytes)
  if (pos + 8 > size)
    return std::unexpected("greeting truncated (auth1)");
  g.auth_data.assign(data + pos, data + pos + 8);
  pos += 8;

  // filler
  if (pos + 1 > size)
    return std::unexpected("greeting truncated (filler)");
  pos += 1;

  // capabilities low 2 bytes
  if (pos + 2 > size)
    return std::unexpected("greeting truncated (cap_low)");
  std::uint16_t cap_low = read_u16_le(data + pos);
  pos += 2;

  // charset, status_flags
  if (pos + 3 > size)
    return std::unexpected("greeting truncated (charset)");
  g.charset = data[pos++];
  g.status_flags = read_u16_le(data + pos);
  pos += 2;

  // capabilities high 2 bytes
  if (pos + 2 > size)
    return std::unexpected("greeting truncated (cap_high)");
  std::uint16_t cap_high = read_u16_le(data + pos);
  pos += 2;
  ...
  if (pos + 1 > size)
    return std::unexpected("greeting truncated (auth_len)");
  std::uint8_t auth_data_len = data[pos++];

  // reserved (10 bytes)
  if (pos + 10 > size)
    return std::unexpected("greeting truncated (reserved)");
  pos += 10;
  ...
  if (g.capabilities & CLIENT_SECURE_CONNECTION) {
    ...
    if (auth_data_len > 8)
      part2_len =
          std::max(part2_len, static_cast<std::size_t>(auth_data_len - 8));
    if (pos + part2_len > size)
      part2_len = size - pos;
    std::size_t actual = part2_len;
    if (actual > 0 && data[pos + actual - 1] == 0)
      --actual;
...
auto parse_ok_packet(const std::uint8_t *data, std::size_t size) -> ok_packet {
  ...
  std::size_t pos = 0;
  if (pos < size)
    ++pos; // skip header (0x00 or 0xFE)

  auto aff = read_lenenc(data + pos, size - pos);
  if (aff.bytes_consumed > 0) {
    ok.affected_rows = aff.value;
    pos += aff.bytes_consumed;
  }

  auto lid = read_lenenc(data + pos, size - pos);
  if (lid.bytes_consumed > 0) {
    ok.last_insert_id = lid.value;
    pos += lid.bytes_consumed;
  }

  if (pos + 2 <= size) {
    ok.status_flags = read_u16_le(data + pos);
    pos += 2;
  }
  if (pos + 2 <= size) {
    ok.warnings = read_u16_le(data + pos);
    pos += 2;
  }

  if (pos < size)
    ok.info =
        std::string(reinterpret_cast<const char *>(data + pos), size - pos);
...
auto parse_err_packet(const std::uint8_t *data, std::size_t size)
    -> err_packet {
  err_packet ep;
  std::size_t pos = 0;
  if (pos < size)
  ...
  if (pos + 2 <= size) {
    ep.error_code = read_u16_le(data + pos);
    pos += 2;
  }
...
auto parse_column_def(const std::uint8_t *data, std::size_t size)
  ...
  if (cat.bytes_consumed > 0)
    pos += cat.bytes_consumed;

  // schema
  auto db = read_lenenc_string(data + pos, size - pos);
  if (db.bytes_consumed > 0) {
    col.database = std::string(db.value);
    pos += db.bytes_consumed;
  }

  // table
  auto tbl = read_lenenc_string(data + pos, size - pos);
  if (tbl.bytes_consumed > 0) {
    col.table = std::string(tbl.value);
    pos += tbl.bytes_consumed;
  }

  // org_table
  auto otbl = read_lenenc_string(data + pos, size - pos);
  if (otbl.bytes_consumed > 0) {
    col.org_table = std::string(otbl.value);
    pos += otbl.bytes_consumed;
  }

  // name
  auto nm = read_lenenc_string(data + pos, size - pos);
  if (nm.bytes_consumed > 0) {
    col.name = std::string(nm.value);
    pos += nm.bytes_consumed;
  }

  // org_name
  auto onm = read_lenenc_string(data + pos, size - pos);
  if (onm.bytes_consumed > 0) {
    col.org_name = std::string(onm.value);
    pos += onm.bytes_consumed;
  }

  // fixed_fields length (lenenc, typically 0x0C)
  auto flen = read_lenenc(data + pos, size - pos);
  if (flen.bytes_consumed > 0)
    pos += flen.bytes_consumed;

  // charset(2) + column_length(4) + type(1) + flags(2) + decimals(1) +
  // filler(2) = 12
  if (pos + 12 <= size) {
    col.charset = read_u16_le(data + pos);
    pos += 2;
    col.column_length = read_u32_le(data + pos);
    pos += 4;
    col.type = static_cast<field_type>(data[pos]);
    pos += 1;
    col.flags = read_u16_le(data + pos);
    pos += 2;
...
auto parse_text_row(const std::uint8_t *data, std::size_t size,
  ...
  for (std::size_t i = 0; i < columns.size(); ++i) {
    ...
    field_value fv;
    fv.kind_ = compute_field_kind(columns[i].type, columns[i].flags,
                                  columns[i].charset);
    ...
    case field_kind::int64: {
      auto [ptr, ec] =
          std::from_chars(raw.data(), raw.data() + raw.size(), fv.int_val);
      (void)ptr;
      (void)ec;
      break;
    }
    case field_kind::uint64: {
      auto [ptr, ec] =
          std::from_chars(raw.data(), raw.data() + raw.size(), fv.uint_val);
      (void)ptr;
      (void)ec;
      break;
    }
    ...
    case field_kind::datetime: {
      ...
      int n = std::sscanf(raw.c_str(), "%u-%u-%u %u:%u:%u.%u", &y, &mo, &d, &h,
                          &mi, &s, &us);
      if (n >= 6) {
    ...
    case field_kind::time: {
      ...
      if (n >= 3) {
        fv.time_val = {neg, h, static_cast<std::uint8_t>(m),
                       static_cast<std::uint8_t>(s), n >= 4 ? us : 0u};
      }
      break;
    }
...
auto parse_binary_row(const std::uint8_t *data, std::size_t size,
  ...
  for (std::size_t i = 0; i < ncols; ++i) {
    ...
    case field_type::tiny:
      ...
      if (is_unsigned)
        fv.uint_val = data[pos];
      else
        fv.int_val = static_cast<std::int8_t>(data[pos]);
      pos += 1;
      break;
    ...
    case field_type::int24:
      ...
      if (is_unsigned)
        fv.uint_val = read_u32_le(data + pos);
      else
        fv.int_val = static_cast<std::int32_t>(read_u32_le(data + pos));
      pos += 4;
      break;
    ...
    case field_type::longlong:
      ...
      if (is_unsigned)
        fv.uint_val = read_u64_le(data + pos);
      else
        fv.int_val = static_cast<std::int64_t>(read_u64_le(data + pos));
      pos += 8;
      break;
...
auto parse_prepare_stmt_response(const std::uint8_t *data, std::size_t size)
  ...
  // status(1=0x00) + stmt_id(4) + num_columns(2) + num_params(2) + reserved(1)
  // + warning_count(2) = 12
  if (size < 12)
    return std::unexpected("prepare response too short");
  ...
  r.stmt_id = read_u32_le(data + pos);
  pos += 4;
  r.num_columns = read_u16_le(data + pos);
  pos += 2;
  r.num_params = read_u16_le(data + pos);
  pos += 2;
  pos += 1; // reserved
  r.warnings = read_u16_le(data + pos);
  pos += 2;
...
auto classify_handshake_response(const std::uint8_t *data, std::size_t size)
  ...
  switch (data[0]) {
  case OK_HEADER:
    return handshake_response_type::ok;
  case ERR_HEADER:
    return handshake_response_type::error;
  case AUTH_SWITCH: // 0xFE — also EOF, but in handshake context it's
                    // auth_switch
