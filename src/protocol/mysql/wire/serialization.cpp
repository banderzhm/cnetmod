...
auto build_login_packet(const connect_options &opts,
  ...
  pkt.reserve(128 + opts.username.size() + opts.database.size() +
              auth_response.size());
...
auto build_prepare_stmt_command(std::string_view sql)
  ...
  pkt.insert(pkt.end(), reinterpret_cast<const std::uint8_t *>(sql.data()),
  ...
  for (auto &p : params) {
    ...
    case param_value::kind_t::datetime_kind: {
      ...
      if (dt.year == 0 && dt.month == 0 && dt.day == 0 && dt.hour == 0 &&
          dt.minute == 0 && dt.second == 0 && dt.microsecond == 0) {
        pkt.push_back(0);
        ...
        pkt.push_back(buf[0]);
        pkt.push_back(buf[1]);
        pkt.push_back(dt.month);
        pkt.push_back(dt.day);
        pkt.push_back(dt.hour);
        pkt.push_back(dt.minute);
        pkt.push_back(dt.second);
