...
auto field_value::kind() const noexcept -> field_kind
{
    return kind_;
}
...
auto field_value::get_int64() const noexcept -> std::int64_t
{
    return int_val;
}
...
auto field_value::get_float() const noexcept -> float
{
    return float_val;
}

auto field_value::get_double() const noexcept -> double
{
    return double_val;
}
...
auto field_value::null() -> field_value
{
    return {};
}
...
auto result_set::is_err() const noexcept -> bool
{
    return !ok();
}

auto result_set::has_rows() const noexcept -> bool
{
    return !rows.empty();
}

auto statement::valid() const noexcept -> bool
{
    return id != 0;
}

auto param_value::null() -> param_value
{
    return {};
}
...
void execution_state::set_state(state_t s) noexcept
{
    state_ = s;
}
