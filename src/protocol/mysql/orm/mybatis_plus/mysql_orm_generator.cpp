    ...
    -> task<std::expected<table_info, std::string>>
{
    ...
    const auto comment_sql =
        std::format("SELECT TABLE_COMMENT FROM information_schema.TABLES " "WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '{}'",
            table_name);
    ...
    const auto columns_sql =
        std::format("SELECT COLUMN_NAME, COLUMN_TYPE, IS_NULLABLE, COLUMN_KEY, " "EXTRA, COLUMN_DEFAULT, COLUMN_COMMENT " "FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = " "DATABASE() AND TABLE_NAME = '{}' " "ORDER BY ORDINAL_POSITION",
            table_name);
