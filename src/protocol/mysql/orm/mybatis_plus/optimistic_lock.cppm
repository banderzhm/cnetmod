...
public:
  ...
  template <Model T> auto has_version_field() const -> bool {
  ...
  /// To:         UPDATE users SET name = ?, version = version + 1 WHERE id = ?
  /// AND version = ?
  template <Model T>
  auto inject_version_check(std::string sql, std::vector<param_value> &params,
    ...
    if (!current_version)
      return sql;
    return inject_version_check_impl(std::move(sql), params, *version_field,
                                     *current_version);
  }

  /// Update entity version after successful UPDATE
  template <Model T> auto increment_version(T &entity) const -> void {
    auto &meta = model_traits<T>::meta();
    for (auto &field : meta.fields) {
      if (has_flag(field.col.flags, VERSION) ||
          field.col.column_name == "version" ||
  ...
  /// If affected_rows == 0, it means version mismatch (optimistic lock
  /// conflict)
  static auto check_update_result(std::uint64_t affected_rows) -> bool;

private:
  static auto inject_version_check_impl(std::string sql,
...
auto update_with_version_check(client &cli, T &entity) -> task<bool> {
  ...
  for (auto &field : meta.fields) {
    ...
    if (has_flag(field.col.flags, col_flag::auto_increment))
      continue;
    if (has_flag(field.col.flags, VERSION))
      continue; // Skip version field in SET
