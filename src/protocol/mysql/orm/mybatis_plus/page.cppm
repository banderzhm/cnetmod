...
export template <Model T> struct page {
  ...
  auto has_next() const noexcept -> bool { return current_page < total_pages; }

  auto has_previous() const noexcept -> bool { return current_page > 1; }

  auto is_first() const noexcept -> bool { return current_page == 1; }

  auto is_last() const noexcept -> bool { return current_page == total_pages; }
};
...
public:
  ...
  template <Model T>
  static auto select_page(client &cli, std::int64_t page_num,
    ...
    auto final_count_sql =
        format_sql(cli.current_format_opts(), count_sql, count_params);
    if (!final_count_sql)
      co_return result;

    auto count_rs = co_await cli.execute(*final_count_sql);
    if (count_rs.is_err() || count_rs.rows.empty())
      co_return result;
    ...
    auto final_select_sql =
        format_sql(cli.current_format_opts(), select_sql, select_params);
    if (!final_select_sql)
      co_return result;
  ...
  static auto select_page(client &cli, std::int64_t page_num,
    ...
    co_return co_await select_page<T>(cli, page_num, page_size,
                                      query_wrapper<T>{});
  }
};
