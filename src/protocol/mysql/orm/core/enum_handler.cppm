...
export template <EnumType E> class enum_handler {
public:
...
export template <EnumType E> class int_enum_handler : public enum_handler<E> {
public:
...
private:
  ...
  template <EnumType E> class enum_handler_wrapper : public enum_handler_base {
  public:
    explicit enum_handler_wrapper(std::unique_ptr<enum_handler<E>> handler)
        : handler_(std::move(handler)) {}

    auto get() const -> const enum_handler<E> * { return handler_.get(); }
...
public:
  template <EnumType E> void register_int_enum() {
  ...
  template <EnumType E>
  void
  register_string_enum(typename string_enum_handler<E>::enum_map_t mapping) {
    auto type_id = std::type_index(typeid(E));
    handlers_[type_id] = std::make_unique<enum_handler_wrapper<E>>(
        std::make_unique<string_enum_handler<E>>(std::move(mapping)));
  }

  template <EnumType E> auto get_handler() const -> const enum_handler<E> * {
    auto type_id = std::type_index(typeid(E));
...
private:
  std::unordered_map<std::type_index, std::unique_ptr<enum_handler_base>>
      handlers_;
};

export auto global_enum_registry() -> enum_registry &;
