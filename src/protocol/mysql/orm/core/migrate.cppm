...
/// Convert ORM column_type + strategy to expected MySQL type prefix (for loose
/// comparison)
...
/// Loose type matching: check if actual type starts with expected prefix
/// (ignore display width)
inline auto type_matches(std::string_view actual, std::string_view expected)
    -> bool {
...
inline auto build_add_column_ddl(std::string_view table, const column_def &col)
    -> std::string {
...
export template <Model T> auto sync_schema(client &cli) -> task<sync_result> {
  ...
  if (rs.is_err()) {
    ...
    if (rs.error_code == 1146 ||
        rs.error_msg.find("doesn't exist") != std::string::npos) {
  ...
  for (auto &chg : result.diff.changes) {
    ...
    if (ar.is_err()) {
      result.error_msg =
          "ALTER failed on `" + chg.column_name + "`: " + ar.error_msg;
      co_return result;
    }
  }
