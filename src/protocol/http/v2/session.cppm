...
using server_handler = std::function<cnetmod::task<server_response>(server_request)>;
using transport_reader = std::function<cnetmod::task<std::expected<std::size_t, std::error_code>>(cnetmod::mutable_buffer)>;
using transport_writer = std::function<cnetmod::task<std::expected<void, std::error_code>>(cnetmod::const_buffer)>;
...
public:
    ...
    [[nodiscard]] auto run(std::span<const std::byte> initial = {}) -> cnetmod::task<void>;
...
private:
    [[nodiscard]] auto process_frame(frame_header header,
        ...
        -> std::expected<void, std::error_code>;
    [[nodiscard]] auto validate_headers(std::span<const header_field> fields) const
        -> std::error_code;
