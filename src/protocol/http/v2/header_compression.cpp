...
namespace {
struct static_entry { std::string_view name; std::string_view value; };
...
void encode_integer(std::vector<std::byte>& out, std::uint32_t value, unsigned prefix,
                    std::uint8_t head = 0) {
    const auto mask = (1u << prefix) - 1u;
    if (value < mask) { out.push_back(static_cast<std::byte>(head | value)); return; }
...
auto decode_integer(std::span<const std::byte> input, std::size_t& offset, unsigned prefix)
    -> std::optional<std::uint32_t> {
    if (offset == input.size()) return std::nullopt;
    const auto mask = (1u << prefix) - 1u;
    auto value = std::uint32_t{std::to_integer<std::uint8_t>(input[offset++]) & mask};
    if (value < mask) return value;
    unsigned shift = 0;
    while (offset < input.size() && shift <= 28) {
        const auto byte = std::to_integer<std::uint8_t>(input[offset++]);
        if (shift == 28 && byte > 0x7f) return std::nullopt;
...
auto decode_string(std::span<const std::byte> input, std::size_t& offset)
    ...
    if (offset == input.size()) return std::nullopt;
    const bool huffman = (std::to_integer<std::uint8_t>(input[offset]) & 0x80) != 0;
    auto size = decode_integer(input, offset, 7);
    if (!size || *size > input.size() - offset) return std::nullopt;
...
header_compression::header_compression(std::size_t limit) : dynamic_table_limit_(limit) {}
void header_compression::evict() {
    while (dynamic_table_bytes_ > dynamic_table_limit_ && !dynamic_table_.empty()) {
...
void header_compression::insert(entry item) {
    ...
    if (bytes > dynamic_table_limit_) { dynamic_table_.clear(); dynamic_table_bytes_ = 0; return; }
...
auto header_compression::at(std::uint32_t index) const -> std::optional<entry> {
    if (index > 0 && index < std::size(static_table))
        return entry{std::string(static_table[index].name), std::string(static_table[index].value)};
    const auto dynamic = static_cast<std::size_t>(index) - std::size(static_table);
    if (index >= std::size(static_table) && dynamic < dynamic_table_.size()) return dynamic_table_[dynamic];
    return std::nullopt;
}
auto header_compression::find(std::string_view name, std::string_view value, bool exact) const
    -> std::uint32_t {
    for (std::uint32_t i = 1; i < std::size(static_table); ++i)
        if (static_table[i].name == name && (!exact || static_table[i].value == value)) return i;
    for (std::size_t i = 0; i < dynamic_table_.size(); ++i)
        if (dynamic_table_[i].name == name && (!exact || dynamic_table_[i].value == value))
            return static_cast<std::uint32_t>(std::size(static_table) + i);
    return 0;
}
void header_compression::set_dynamic_table_limit(std::size_t bytes) { dynamic_table_limit_ = bytes; evict(); }
auto header_compression::dynamic_table_limit() const noexcept -> std::size_t { return dynamic_table_limit_; }
auto header_compression::decode(std::span<const std::byte> block)
    ...
    while (offset < block.size()) {
        ...
        if (first & 0x80) {
            ...
            if (!value) return std::unexpected(std::make_error_code(std::errc::protocol_error));
            result.push_back({std::move(value->name), std::move(value->value)});
            continue;
        }
        if ((first & 0xe0) == 0x20) {
            if (!can_resize) return std::unexpected(std::make_error_code(std::errc::protocol_error));
            auto size = decode_integer(block, offset, 5);
            if (!size) return std::unexpected(std::make_error_code(std::errc::protocol_error));
        ...
        if (!index) return std::unexpected(std::make_error_code(std::errc::protocol_error));
        std::string name;
        if (*index) {
            auto value = at(*index);
            if (!value) return std::unexpected(std::make_error_code(std::errc::protocol_error));
            ...
            if (!value || value->empty()) return std::unexpected(std::make_error_code(std::errc::protocol_error));
            name = std::move(*value);
        }
        auto value = decode_string(block, offset);
        if (!value) return std::unexpected(std::make_error_code(std::errc::protocol_error));
        header_field field{std::move(name), std::move(*value), sensitive};
        if (incremental) insert({field.name, field.value});
...
auto header_compression::encode(std::span<const header_field> fields)
    ...
    for (const auto& field : fields) {
        if (field.name.empty()) return std::unexpected(std::make_error_code(std::errc::invalid_argument));
        ...
        encode_integer(result, name, indexed ? 6 : 4, indexed ? 0x40 : (field.sensitive ? 0x10 : 0));
        if (!name) encode_string(result, field.name);
        encode_string(result, field.value);
        if (indexed) insert({field.name, field.value});
    }
    return result;
}
} // namespace cnetmod::http::v2
