...
export inline auto body_limit(std::size_t max_bytes = 1024 * 1024)
    -> http::middleware_fn
{
    return [max_bytes](http::request_context& ctx, http::next_fn next) -> task<void>
    {
        ...
        if (!cl.empty())
        {
            ...
            {
                ctx.json(http::status::payload_too_large, std::format(R"({{"error":"request body too large","limit":{},"size":{}}})", max_bytes, len));
                co_return;
            }
        }
        ...
        {
            ctx.json(http::status::payload_too_large, std::format(R"({{"error":"request body too large","limit":{},"size":{}}})", max_bytes, ctx.body().size()));
            co_return;
        }
