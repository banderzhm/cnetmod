...
auto serve_dir_body(const static_file_options &opts, request_context &ctx)
  ...
  if (is_range) {
    ...
    resp.set_header("Content-Range", std::format("bytes {}-{}/{}", range_start,
                                                 range_end, file_size));
...
auto save_upload_body(const upload_options &opts, request_context &ctx)
  ...
  if (ct_parsed.mime == "multipart/form-data") {
    ...
    if (!form_r) {
      ctx.text(status::bad_request, std::format("Multipart parse error: {}",
    ...
    for (auto &ff : form.all_files()) {
      ...
      auto f = file::open(full_path, open_mode::write | open_mode::create |
                                         open_mode::truncate);
      if (!f)
        continue;
  ...
  ctx.json(status::ok, std::format(R"({{"filename":"{}","size":{}}})", filename,
                                   body.size()));
  co_return;
}
...
void server::set_router(router r) { router_ = std::move(r); }

void server::use(middleware_fn mw) { middlewares_.push_back(std::move(mw)); }

void server::set_max_connections(std::size_t n) { max_connections_ = n; }
