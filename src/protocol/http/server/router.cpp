...
auto is_static_route(const std::vector<segment> &segs) noexcept -> bool {
  return std::ranges::all_of(
      segs, [](const segment &s) { return s.kind == segment_kind::exact; });
}
...
request_context::request_context(io_context &c, socket &s,
                                 ...
                                 route_params x)
    : ctx_(c), sock_(s), resp_(r), params_(std::move(x)),
      headers_ptr_(&p.headers()), method_(p.method()), body_(p.body()) {
...
request_context::request_context(io_context &c, socket &s, std::string_view m,
    ...
    : ctx_(c), sock_(s), resp_(r), params_(std::move(x)), headers_ptr_(&h),
      method_(m), body_storage_(b), body_(body_storage_),
...
auto request_context::uri() const noexcept -> std::string_view { return uri_; }
...
void request_context::not_found() { text(status::not_found, "404 Not Found"); }
...
auto request_context::resp() noexcept -> response & { return resp_; }
auto request_context::io_ctx() noexcept -> io_context & { return ctx_; }
auto request_context::raw_socket() noexcept -> socket & { return sock_; }
