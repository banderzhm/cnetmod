...
auto swagger_ui_html(std::string_view openapi_url, std::string_view title)
    -> std::string {
...
<script>window.ui=SwaggerUIBundle({{url:"{}",dom_id:"#swagger-ui",deepLinking:true,presets:[SwaggerUIBundle.presets.apis],layout:"BaseLayout"}});</script>
                     ...
                     escape_html_json(title), escape_html_json(openapi_url));
}

auto swagger_ui_handler(std::string openapi_url, std::string title)
    -> handler_fn {
