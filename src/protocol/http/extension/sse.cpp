...
auto comment(std::string_view t) -> std::string
{
    return encode({.comment = std::string(t)});
}
...
void prepare(response& r, response_options o)
{
    ...
    if (o.no_cache)
        r.set_header("Cache-Control", "no-cache, no-transform");
    if (o.keep_alive)
        r.set_header("Connection", "keep-alive");
    if (o.disable_proxy_buffering)
