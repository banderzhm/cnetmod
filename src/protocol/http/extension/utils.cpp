...
auto resolve_client_ip(const request_context& ctx, std::string_view fallback) -> std::string
{
    if (auto x = ctx.get_header("X-Forwarded-For"); !x.empty())
    {
        auto p = x.substr(0, x.find(','));
    ...
    if (auto x = ctx.get_header("X-Real-IP"); !x.empty())
        return std::string(x);
