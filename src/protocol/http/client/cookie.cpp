...
auto parse_http_date(std::string_view date_str)
  ...
  while (!date_str.empty() &&
         (date_str.front() == ' ' || date_str.front() == '-')) {
    date_str.remove_prefix(1);
  }
  ...
  if (space2 == std::string_view::npos &&
      date_str.find('-') != std::string_view::npos) {
  ...
  for (std::size_t i = 0; i < months.size(); ++i) {
    if (month_str.size() >= 3 &&
        std::equal(months[i].begin(), months[i].end(), month_str.begin(),
                   month_str.begin() + 3, [](char a, char b) {
  ...
  int hour = 0;
  auto [ptr3, ec3] =
      std::from_chars(hour_str.data(), hour_str.data() + hour_str.size(), hour);
  if (ec3 != std::errc{})
    return std::nullopt;
  tm.tm_hour = hour;
  ...
  auto sec_str = (space4 != std::string_view::npos) ? date_str.substr(0, space4)
                                                    : date_str;

  int sec = 0;
  auto [ptr5, ec5] =
      std::from_chars(sec_str.data(), sec_str.data() + sec_str.size(), sec);
  if (ec5 != std::errc{})
    return std::nullopt;
  tm.tm_sec = sec;
...
auto cookie::matches_domain(std::string_view host) const -> bool {
  ...
  if (domain.starts_with('.')) {
    ...
    if (host == domain_without_dot ||
        host.ends_with("." + std::string(domain_without_dot))) {
      return true;
    }
  } else {
...
auto cookie::matches_path(std::string_view request_path) const -> bool {
  ...
  if (request_path.starts_with(path)) {
    ...
    if (path.ends_with('/') || request_path.size() == path.size() ||
...
auto cookie::to_cookie_header() const -> std::string {
  return name + "=" + value;
}

auto cookie::is_expired() const -> bool {
  return expires && std::chrono::system_clock::now() > *expires;
}

void cookie_jar::add(const cookie &value) {
  std::erase_if(cookies_, [&value](const cookie &existing) {
    return existing.name == value.name && existing.domain == value.domain &&
...
void cookie_jar::add_from_header(std::string_view header) {
  if (const auto value = cookie::parse(header))
    add(*value);
}

auto cookie_jar::get_cookies(std::string_view host, std::string_view path,
                             bool is_secure) const -> std::vector<cookie> {
  std::vector<cookie> result;
  for (const auto &value : cookies_) {
    if (!value.is_expired() && (!value.secure || is_secure) &&
        value.matches_domain(host) && value.matches_path(path)) {
...
auto cookie_jar::to_cookie_header(std::string_view host, std::string_view path,
                                  bool is_secure) const -> std::string {
  const auto values = get_cookies(host, path, is_secure);
  std::string result;
  for (const auto &value : values) {
    if (!result.empty())
      result += "; ";
    result += value.to_cookie_header();
  }
  return result;
}

void cookie_jar::clear() { cookies_.clear(); }

void cookie_jar::clear_expired() {
  std::erase_if(cookies_,
                [](const cookie &value) { return value.is_expired(); });
}

auto cookie_jar::cookies() const -> const std::vector<cookie> & {
  return cookies_;
}
