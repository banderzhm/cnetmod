...
export namespace cnetmod::http
{
    ...
    /// HTTP/2 clients multiplex streams while HTTP/1 clients are leased exclusively.
    struct client_pool_key
    {
        ...
        [[nodiscard]] auto operator==(const client_pool_key&) const noexcept -> bool = default;
        [[nodiscard]] auto operator<=>(const client_pool_key&) const noexcept = default;
    };
    ...
    private:
        ...
        std::map<client_pool_key, std::vector<std::unique_ptr<client>>> endpoint_idle_;
    };
