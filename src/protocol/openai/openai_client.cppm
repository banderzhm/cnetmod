...
/// Reuses cnetmod's ssl_context / ssl_stream / http::request /
...
public:
  ...
  /// Streaming Chat Completions — SSE chunk-by-chunk callback, returns complete
  /// concatenated result
...
private:
  ...
  [[nodiscard]] static auto get_audio_content_type(std::string_view filename)
      -> std::string;
  ...
  [[nodiscard]] auto
  build_image_variation_form(const image_variation_request &req)
      -> std::pair<std::string, std::string>;
