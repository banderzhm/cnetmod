...
// Protocol-qualified filename prevents duplicate archive member names.
...
auto embedding_response::from_json(std::string_view text)
    -> embedding_response {
  ...
  if (j.contains("results") && j["results"].is_array()) {
    for (auto &res : j["results"]) {
      ...
      if (res.contains("categories") && res["categories"].is_object()) {
        ...
        mr.categories.harassment_threatening =
            c.value("harassment/threatening", false);
        mr.categories.self_harm = c.value("self-harm", false);
        mr.categories.self_harm_intent = c.value("self-harm/intent", false);
        mr.categories.self_harm_instructions =
            c.value("self-harm/instructions", false);
