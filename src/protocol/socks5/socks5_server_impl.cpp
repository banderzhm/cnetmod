...
auto read_gssapi_message(io_context& io, socket& sock)
    ...
    if (auto read = co_await read_exact(
            io, sock, mutable_buffer{prefix.data(), prefix.size()}); !read) {
    ...
    if (auto read = co_await read_exact(
            io, sock,
            mutable_buffer{length_bytes.data(), length_bytes.size()}); !read) {
        co_return std::unexpected(read.error());
    }
    const auto length =
    ...
    if (length != 0) {
        if (auto read = co_await read_exact(
                io, sock,
                mutable_buffer{message.token.data(), message.token.size()}); !read) {
            co_return std::unexpected(read.error());
        }
    }
    co_return message;
...
auto write_gssapi_message(io_context& io, socket& sock,
    ...
    if (!serialized) co_return std::unexpected(serialized.error());
...
auto write_client_payload(io_context& io, socket& client,
    ...
    if (!wrapped) co_return std::unexpected(wrapped.error());
...
auto server::handle_authentication(socket& client, io_context& io) 
    ...
    if (selected == auth_method::gssapi) {
        ...
        if (!requested) co_return std::unexpected(requested.error());
        ...
        co_return std::optional<gssapi_session>{gssapi_session{
            .context = std::move(*context), .protection = selected_level}};
    }
