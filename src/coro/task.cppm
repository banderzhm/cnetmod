...
template <typename T, typename Promise> struct task_awaiter {
...
/// void specialization
template <typename Promise> struct task_awaiter<void, Promise> {
  std::coroutine_handle<Promise> h;
  auto await_ready() const noexcept -> bool { return false; }
  auto await_suspend(std::coroutine_handle<> caller) noexcept
      -> std::coroutine_handle<> {
...
export template <typename T> class task {
public:
...
/// when_all subtask wrapper — notify state at final_suspend instead of resuming
/// caller
...
public:
  ...
  when_all_task(when_all_task &&o) noexcept
...
private:
  explicit when_all_task(std::coroutine_handle<promise_type> h) noexcept
      : handle_(h) {}
  std::coroutine_handle<promise_type> handle_;
};

/// void specialization
template <> class when_all_task<void> {
public:
  struct promise_type {
    when_all_state *state_ = nullptr;
    std::exception_ptr exception_;
...
template <typename T> auto make_when_all_task(task<T> t) -> when_all_task<T> {
  co_return co_await std::move(t);
}

auto make_when_all_task(task<void> t) -> when_all_task<void>;

/// when_all awaiter: suspend caller and concurrently start all subtasks
template <typename... Tasks> struct when_all_awaiter {
  when_all_state state_;
  std::tuple<Tasks...> tasks_;

  explicit when_all_awaiter(Tasks... ts)
      : state_(sizeof...(Tasks)), tasks_(std::move(ts)...) {}
  ...
  void await_suspend(std::coroutine_handle<> caller) noexcept {
    ...
    std::apply([this](auto &...t) { (t.set_state(&state_), ...); }, tasks_);
    std::apply([](auto &...t) { (t.start(), ...); }, tasks_);
  }
...
auto when_all(task<T1> t1, task<T2> t2) -> task<std::tuple<T1, T2>> {
  auto w1 = detail::make_when_all_task(std::move(t1));
  auto w2 = detail::make_when_all_task(std::move(t2));

  detail::when_all_awaiter<detail::when_all_task<T1>, detail::when_all_task<T2>>
      aw{std::move(w1), std::move(w2)};
  co_await aw;
...
auto when_all(task<T1> t1, task<T2> t2, task<T3> t3)
  ...
  detail::when_all_awaiter<detail::when_all_task<T1>, detail::when_all_task<T2>,
                           detail::when_all_task<T3>>
      aw{std::move(w1), std::move(w2), std::move(w3)};
  co_await aw;
...
auto when_all(task<Ts>... ts) -> task<std::tuple<Ts...>> {
  ...
  co_await aw;
  co_return std::apply(
      [](auto &...w) { return std::tuple<Ts...>{w.result()...}; }, aw.tasks_);
}

/// when_all: void + non-void → returns non-void result (void task runs
/// concurrently)
...
auto when_all(task<T1> t1, task<void> t2) -> task<T1> {
  ...
  detail::when_all_awaiter<detail::when_all_task<T1>,
...
export auto when_all(task<void> t1, task<void> t2) -> task<void>;
