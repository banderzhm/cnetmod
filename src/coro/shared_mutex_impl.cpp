...
async_shared_mutex::auto_lock::auto_lock(spinlock& lock) noexcept
    : lk_(lock)
{
    lk_.lock();
}
...
async_shared_mutex::lock_shared_awaitable::lock_shared_awaitable(
    async_shared_mutex& mutex) noexcept
    : rw_(mutex)
{
}
...
void async_shared_mutex::unlock() noexcept
{
    ...
    waiter_node* readers = nullptr;
    {
        ...
        {
            int count = 0;
            for (auto* node = read_head_; node; node = node->next)
                ++count;
            state_ = count;
            readers = read_head_;
...
async_shared_lock_guard::async_shared_lock_guard(
    async_shared_mutex& mutex, std::adopt_lock_t) noexcept
    : rw_(&mutex)
{
}
