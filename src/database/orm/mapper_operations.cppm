export module cnetmod.orm.mapper_operations;

import std;
import cnetmod.orm.database_backend;
...
/**
 * @brief Internal model-operation engine used by typed Mappers.
 *
 * This implementation owns model metadata, SQL generation, typed mapping,
 * batching and model policies. `database_session` privately stores it so the
 * public Session contract remains limited to raw execution and transactions.
 * Application code reaches these operations only through `mapper<T>` and
 * `repository<T>`.
 */
namespace detail {
    ...
    public:
        ...
        {
        public:
            cursor(mapper_operations& owner, query_wrapper<T> query,
        ...
        explicit mapper_operations(Client& client,
    ...
    private:
        ...
        // The public typed surface is cnetmod::orm::mapper<T>. This internal
        // implementation owns model SQL generation while database_session exposes
        // only protocol-neutral execution and transaction operations.
     ...
     */
        ...
        /**
         * @brief Executes a tagged query wrapper directly.
         *
         * A query wrapper defaults to SELECT. Callers opt into DELETE
         * explicitly with `as_delete()`.
         */
        ...
        /**
         * @brief Executes an update wrapper as one SQL operation.
         */
     ...
     */
        ...
        /**
         * @brief Runs an expected-returning operation in a transaction.
         *
         * The operation's value is returned after a successful commit. Any
         * failure attempts a rollback before returning the original error.
         */
        ...
        /**
         * @brief Runs an expected-returning transaction at an isolation level.
         */
        template <typename T, typename Function>
        requires std::invocable<Function&> && requires(Function& function) {
            { function() } -> std::same_as<task<std::expected<T, std::string>>>;
...
namespace detail {
    ...
    /**
     * @brief Bridges Mapper APIs to their private model-operation engine.
     *
     * The bridge keeps model SQL generation out of the public database_session
     * contract without requiring a cross-module mapper<T> friend declaration.
     */
    struct mapper_session_access
    {
    private:
        template <class Session>
        static auto operations(Session& session) noexcept -> decltype(auto)
        {
            return session.mapper_operations();
        }
    ...
    public:
        ...
        {
            return operations(session).template prepare_select<T>(query);
        }
        ...
        static auto open_cursor(Session& session, query_wrapper<T> query = {},
            cursor_options options = {})
        {
            return operations(session).template open_cursor<T>(
        ...
        static auto select_by_map(Session& session,
            std::span<const std::pair<std::string, param_value>> values)
        {
            return operations(session).template find_by_map<T>(values);
        }

        template <Model T, class Session>
        static auto exists(Session& session, const query_wrapper<T>& query)
        {
            return operations(session).template exists<T>(query);
        }

        template <Model T, class Session>
        static auto count(Session& session, const query_wrapper<T>& query)
        {
            return operations(session).template count_result<T>(query);
        }
        ...
        {
            return operations(session).template page<T>(page, page_size, query);
        }

        template <Model T, class Session>
        static auto select_maps(Session& session, const query_wrapper<T>& query)
        {
            return operations(session).template select_maps<T>(query);
        }

        template <Model T, class Session>
        static auto select_objects(Session& session, const query_wrapper<T>& query)
        {
            return operations(session).template select_objects<T>(query);
        }
        ...
        static auto select_maps_page(Session& session, std::size_t page,
            std::size_t page_size, const query_wrapper<T>& query)
        {
            return operations(session).template page_maps<T>(
        ...
        {
            return operations(session).insert(model);
        }

        template <Model T, class Session>
        static auto update_by_id(Session& session, T& model)
        {
            return operations(session).update(model);
        }
        ...
        {
            return operations(session).template update<T>(query);
        }
        ...
        {
            return operations(session).template update<T>(query, permission);
        }

        template <Model T, class Session>
        static auto save_or_update(Session& session, T& model)
        {
            return operations(session).template save_or_update<T>(model);
        }

        template <Model T, class Session>
        static auto upsert(Session& session, T& model)
        {
            return operations(session).template upsert<T>(model);
        }

        template <Model T, class Session>
        static auto remove(Session& session, const query_wrapper<T>& query)
        {
            return operations(session).template remove<T>(query);
        }
        ...
        static auto remove(Session& session, const query_wrapper<T>& query,
            allow_full_table_t permission)
        {
            return operations(session).template remove<T>(query, permission);
        }

        template <Model T, class Session>
        static auto remove_by_id(Session& session, param_value id)
        {
            return operations(session).template remove_by_id<T>(std::move(id));
        }

        template <Model T, class Session, typename Id>
        static auto remove_by_ids(Session& session, std::span<const Id> ids)
        {
            return operations(session).template remove_by_ids<T>(ids);
        }
        ...
        static auto remove_by_map(Session& session,
            std::span<const std::pair<std::string, param_value>> values)
        {
            return operations(session).template remove_by_map<T>(values);
        }
        ...
        {
            return operations(session).template insert_batch<T>(models, batch_size);
        }
        ...
        static auto update_batch(Session& session, std::span<const T> models,
            std::size_t batch_size)
        {
            return operations(session).template update_batch_by_id<T>(
                models, batch_size);
        }
        ...
        static auto save_or_update_batch(Session& session, std::span<T> models,
            std::size_t batch_size)
        {
            return operations(session).template save_or_update_batch<T>(
                models, batch_size);
        }
        ...
        static auto upsert_batch(Session& session, std::span<T> models,
            std::size_t batch_size)
        {
            return operations(session).template upsert_batch<T>(models, batch_size);
        }
        ...
        {
            return operations(session).template for_each<T>(
        ...
        {
            return operations(session).template for_each<T>(query,
        ...
        static auto for_each_map(Session& session, const query_wrapper<T>& query,
            Handler handler, stream_options options)
        {
            return operations(session).template for_each_map<T>(
                query, std::move(handler), options);
        }
        ...
        static auto for_each_map(Session& session, const query_wrapper<T>& query,
            Handler handler, stream_options options, cancel_token& cancellation)
        {
            return operations(session).template for_each_map<T>(query,
                std::move(handler), options, cancellation);
        }
    };
