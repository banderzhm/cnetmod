module cnetmod.orm.expression_engine;
...
auto expr_value::make_null() -> expr_value
{
    return {};
}
...
expr_lexer::expr_lexer(std::string_view s) noexcept
    : src_(s) {}
...
expr_parser::expr_parser(std::string_view s)
    : lexer_(s)
{
    advance();
}
...
auto expr_parser::parse() -> std::unique_ptr<ast_node>
{
    return parse_or();
}
