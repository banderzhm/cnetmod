module cnetmod.orm.query_cache;
...
query_cache::query_cache(cache_config config)
...
auto std::hash<cnetmod::orm::cache_key>::operator()(
    const cnetmod::orm::cache_key& key) const noexcept -> std::size_t
{
    ...
    for (const auto& param : key.params)
    {
        ...
        switch (param.kind)
        {
        case cnetmod::orm::param_value::kind_t::int64_kind:
            value = std::hash<std::int64_t>{}(param.int_val);
            break;
        case cnetmod::orm::param_value::kind_t::uint64_kind:
            value = std::hash<std::uint64_t>{}(param.uint_val);
            break;
        case cnetmod::orm::param_value::kind_t::double_kind:
            value = std::hash<double>{}(param.double_val);
            break;
        case cnetmod::orm::param_value::kind_t::string_kind:
        case cnetmod::orm::param_value::kind_t::blob_kind:
