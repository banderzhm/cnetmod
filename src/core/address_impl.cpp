...
endpoint::endpoint(ip_address addr, std::uint16_t port) noexcept
    : address_(addr), port_(port) {}

auto endpoint::address() const noexcept -> const ip_address & {
  return address_;
}

auto endpoint::port() const noexcept -> std::uint16_t { return port_; }

void endpoint::set_address(ip_address addr) noexcept { address_ = addr; }

void endpoint::set_port(std::uint16_t port) noexcept { port_ = port; }
